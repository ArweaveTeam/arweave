...
tiny_network_with_reward_pool_test() ->
	...
	?assertEqual((hd(ar_storage:read_block(Bs2)))#block.height, 2).
...
tiny_blockweave_with_mining_test() ->
	...
	?assertEqual((hd(ar_storage:read_block(B1)))#block.height, 1).
...
tiny_blockweave_with_added_data_test() ->
	...
	?assertEqual((hd(ar_storage:read_block(B1)))#block.txs, [TestDataID]).
