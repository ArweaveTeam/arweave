-module(ar_block).
-export([new/0]).
-export([block_to_binary/1, block_field_size_limit/1, generate_block_data_segment/5]).
-include("ar.hrl").
...
block_field_size_limit(B) ->
    ...
    (byte_size(B#block.last_retarget) =< 12) and
    ...
    (byte_size(list_to_binary(B#block.previous_block)) =< 2048).
...
generate_block_data_segment(CurrentB, RecallB, TXs, unclaimed, Tags) -> 
    generate_block_data_segment(CurrentB, RecallB, TXs, <<>>, Tags);
generate_block_data_segment(CurrentB, RecallB, TXs, RewardAddr, Tags) -> 
