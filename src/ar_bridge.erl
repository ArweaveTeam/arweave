...
send_block_to_external(ExternalPeers, B, Recall) ->
	spawn(fun() ->
		{RecallIndepHash, RecallSize, Key, Nonce, BDS} = Recall,
		ar:report(
			[
		...
		send_block_to_external_parallel(ExternalPeers, B, RecallIndepHash, RecallSize, Key, Nonce, BDS)
	end).
...
%% in order.
send_block_to_external_parallel(Peers, NewB, RecallIndepHash, RecallSize, Key, Nonce, BDS) ->
