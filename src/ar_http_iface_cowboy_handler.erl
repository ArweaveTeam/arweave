...
init(Req0, State) ->
	{Status, Hdrs, Body} = do_handle(Req0),
	CowboyHeaders = normalize_headers(Hdrs),
	Req1 = cowboy_req:reply(Status, CowboyHeaders, Body, Req0),
	{ok, Req1, State}.
...
do_handle(Req) ->
	...
	Peer = cowboy_req_to_peer(Req),
	do_handle(Req, Peer).

do_handle(Req, Peer) ->
	{ok, Method} = method_to_atom(cowboy_req:method(Req)),
	{ok, SplitPath} = split_path(cowboy_req:path(Req)),
...
do_handle(Method, [<<"height">>], _Req) when (Method == 'GET') or (Method == 'HEAD') ->
...
cowboy_req_to_peer(Req) ->
	{{IpV4_1, IpV4_2, IpV4_3, IpV4_4}, Port} = cowboy_req:peer(Req),
	{IpV4_1, IpV4_2, IpV4_3, IpV4_4, Port}.

method_to_atom(<<"HEAD">>) -> {ok, 'HEAD'};
method_to_atom(<<"GET">>) -> {ok, 'GET'};
method_to_atom(<<"POST">>) -> {ok, 'POST'};
method_to_atom(<<"OPTIONS">>) -> {ok, 'OPTIONS'};
method_to_atom(OtherMethod) -> {unsupported, OtherMethod}.
...
normalize_headers(Hdrs) ->
	{_, Normalized} = lists:mapfoldl(
		fun({Name, Value}, Acc) ->
			{not_used, maps:put(string:lowercase(Name), Value, Acc)}
		end,
		#{},
		Hdrs
	),
...
post_block(request, Req) ->
	...
	case request_to_struct_with_blockshadow(Req) of
		...
		{ok, {ReqStruct, BShadow}} ->
			OrigPeer = cowboy_req_to_peer(Req),
