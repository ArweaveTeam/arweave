...
-export([start/0, start/1, new_block/2, uniform_wait/2]).
...
%% The address to watch for update txs.
-define(ADDR, <<>>).

%% @doc Start the autoupdater, optionally with a non-defualt address.
...
process_update(TX) ->
    io:format(
        "~n"
        "UPDATE Tramsaction received. ~n"
        "-- Update name: ~s ~n"
        "~n",
        [TX#tx.data]
    ),
    update(),
    % Wait a random amount of time before the restarating the updatetd node.
