...
%% @doc The hashing algorithm used to verify that the weave has not been tampered with.
...
%% @doc The amount of time to wait before refreshing miner data in case of difficulty change.
...
%% @doc Number of transfers for which not to score (and potentially drop) new peers.
...
-record(block, {
	...
	diff = ?DEFAULT_DIFF, % Puzzle difficulty, the number of zeros that need to preceed the hash?
	...
	weave_size = 0, % The current size of the weave in bytes (only totals tx data fields).
...
%% @doc A message intended to be handled by the gossip protocol library (ar_gossip).
-record(gs_msg, {
	hash,
	data
}).
...
%% Helper macros
%%
%% @doc Macro: Return number of winstons per given AR.
-define(AR(AR), (?WINSTON_PER_AR * AR)).
%% @doc Macro: Return whether an object is a block.
-define(IS_BLOCK(X), (is_record(X, block))).
%% @doc Macro: Return whether a value is an address.
