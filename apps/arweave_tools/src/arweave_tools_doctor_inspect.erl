%%% Offline chunk and storage-module inspection commands.
...
main(Args) ->
    case Args of
        ...
        ["chunks", Dir, StartStr, EndStr | AddrListStr] when length(AddrListStr) >= 1 ->
            ...
            {ok, _} = application:ensure_all_started(arweave_entropy),
            ar_packing_sup:start_link(),
            Start = arweave_constants:get_chunk_padded_offset(list_to_integer(StartStr)),
            End = arweave_constants:get_chunk_padded_offset(list_to_integer(EndStr)),
...
inspect_chunk(Dir, PaddedEndOffset, Addresses) ->
    ...
    {Position, ChunkOffset} =
        arweave_storage:get_position_and_relative_chunk_offset(
...
check_all(ExpectedChunkID, LocalChunk, PaddedEndOffset, Addresses, TXRoot, ChunkSize) ->
    ...
    case LocalID =:= ExpectedChunkID of
        true ->
            {match, "Raw chunk"};
        false ->
            Entropy = arweave_entropy:generate_chunk(
