...
%% The chunks of one replica.2.9 footprint in one store. Entropy is
...
%%
%% The active batch's peer lives in arweave_sync_scheduler's footprints map, not in
...
%% Cached peer ranges returned by arweave_sync_discovery.
-record(peer_range, {
    store_id :: term(),
...
%% arweave_sync_discovery fetches byte-mode chunk intervals at the same granularity,
...
-ifdef(AR_TEST).
% 10 MB
-define(NETWORK_DATA_BUCKET_SIZE, 10_000_000).
-else.
% 10 GB
-define(NETWORK_DATA_BUCKET_SIZE, 10_000_000_000).
-endif.
...
-ifdef(AR_TEST).
% 9 (footprints) * 4 (chunks); ~10 MB
-define(NETWORK_FOOTPRINT_BUCKET_SIZE, 36).
-else.
% 37 (footprints) * 1024 (chunks); ~10 GB
-define(NETWORK_FOOTPRINT_BUCKET_SIZE, 37888).
-endif.
