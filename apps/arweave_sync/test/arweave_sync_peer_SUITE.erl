...
dispatch_tracks_fetch_capacity(_Config) ->
    ...
    ?assertEqual(1 / 4, arweave_sync_peer:load(peer, Dispatches1)),
    Dispatches2 = arweave_sync_peer:start_task(peer, Task, Dispatches1),
    ?assert(arweave_sync_peer:has_capacity(peer, Dispatches1)),
    Dispatches3 = arweave_sync_peer:enqueue_tasks(peer, [Task], Dispatches2),
    ?assertEqual(1 / 2, arweave_sync_peer:load(peer, Dispatches3)),
    Dispatches4 = arweave_sync_peer:start_task(peer, Task, Dispatches3),
    ?assertNot(arweave_sync_peer:has_capacity(peer, Dispatches4)).
...
queued_footprint_tasks_share_peer_queue_with_fetches(_Config) ->
    ...
    ?assertNot(arweave_sync_peer:has_capacity(Reservation, Source, Dispatches1)),
    ?assert(arweave_sync_peer:has_capacity(Peer, Dispatches1)),
    Dispatches2 = arweave_sync_peer:start_task(Peer, Task, Dispatches1),
    ?assertEqual(1 / 2, arweave_sync_peer:load(Peer, Dispatches2)),
    ?assert(arweave_sync_peer:has_capacity(Task, Source, Dispatches2)),
    Dispatches3 = arweave_sync_peer:start_task(Peer, Task, Dispatches2),
    ?assertNot(arweave_sync_peer:has_capacity(Peer, Dispatches3)).
...
start_dispatch_restores_queued_assignment_limit(_Config) ->
    ...
    Dispatches = arweave_sync_peer:start_dispatch(#{}, [Task], State),
...
dispatch_splits_peer_capacity_across_stores(_Config) ->
    ...
    Dispatches1 = arweave_sync_peer:start_task(peer, Task, DispatchesA),
    ?assertEqual(1 / 4, arweave_sync_peer:store_load(peer, store_a, Dispatches1)),
    ?assertEqual(0.0, arweave_sync_peer:store_load(peer, store_b, Dispatches1)),
    ...
    Dispatches2 = arweave_sync_peer:enqueue_tasks(
        peer, [FootprintTask, FootprintTask, FootprintTask], Dispatches1
    ),
    ?assertEqual(
        0,
        arweave_sync_peer:store_capacity(peer, store_a, Dispatches2)
    ).
...
assigned_stores_remain_in_target_split(_Config) ->
    ...
    Dispatches1A = arweave_sync_peer:enqueue_tasks(
        Peer,
        [FootprintTask, FootprintTask, FootprintTask, FootprintTask],
        Dispatches1
    ),
    Dispatches2 = arweave_sync_peer:set_store_task_targets(
        #{Peer => [StoreB]}, Dispatches1A
    ),
    ?assertEqual(0, arweave_sync_peer:store_capacity(Peer, StoreA, Dispatches2)),
    ?assertEqual(0, arweave_sync_peer:store_capacity(Peer, StoreB, Dispatches2)).
...
source_capacity_filter_precedes_load(_Config) ->
    ...
    DispatchesA = arweave_sync_peer:enqueue_tasks(BlockedPeer, [BlockedTask], Dispatches0),
    Dispatches = arweave_sync_peer:start_task(BlockedPeer, BlockedTask, DispatchesA),
    ReadySources = lists:filter(
        fun(Source) ->
            arweave_sync_peer:has_capacity(Source, Dispatches)
        end,
        Sources
    ),
    ...
    ?assertMatch(
        ...
        arweave_sync_peer:best_source(StoreID, ReadySources, Dispatches)
    ).
...
record_result_accumulates_observations(_Config) ->
    ...
    State3 = arweave_sync_peer:tick([Peer], #{Peer => 1}, #{Peer => true}, 1000, State2),
...
update_delivery(_Config) ->
    ...
    D0 = arweave_sync_peer:update_delivery(
        [Fast, Slow],
        #{Fast => 0, Slow => 0},
        #{},
        Driven,
    ...
    D1 = arweave_sync_peer:update_delivery(
        [Fast, Slow],
    ...
    D2 = arweave_sync_peer:update_delivery(
        [Fast],
    ...
    D3 = arweave_sync_peer:update_delivery(
        [Fast],
        #{Fast => 3 * ?DATA_CHUNK_SIZE},
        D2,
        #{},
    ...
    D6 = lists:foldl(
        fun(N, Acc) ->
            arweave_sync_peer:update_delivery(
                [Fast],
                #{Fast => 3 * ?DATA_CHUNK_SIZE},
                Acc,
                Driven,
    ...
    ?assertEqual(0.0, arweave_sync_peer:goodput_rate(undefined)),
    ?assertEqual(1.5, arweave_sync_peer:goodput_rate(1.5)).
...
aggregate_goodput_bounds_peer_sum(_Config) ->
    ...
    Goodput = arweave_sync_peer:bound_aggregate_goodput(#{PeerA => 3.0, PeerB => 2.0}, 4.0),
    ...
    ?assertEqual(
        undefined,
        arweave_sync_peer:aggregate_goodput({[PeerA], {0.0, 0, 1.0}})
    ).
...
aggregate_delivery_smooths_bucket_boundary(_Config) ->
    CS = float(?DATA_CHUNK_SIZE),
    Entry0 = arweave_sync_peer:aggregate_delivery_entry(0, 1, 0, undefined),
    %% Ten chunks in one second seed a ten-chunk/s aggregate estimate.
    Entry1 = arweave_sync_peer:aggregate_delivery_entry(10 * ?DATA_CHUNK_SIZE, 1, 1000, Entry0),
    ?assertEqual(10 * CS / 1000, element(3, Entry1)),
    %% A driven empty second retains four-fifths under the five-sample EWMA.
    Entry2 = arweave_sync_peer:aggregate_delivery_entry(
...
goodput_probe(_Config) ->
    ...
    ?assert(arweave_sync_peer:goodput_improved(100.51, 100.0)),
    ?assertNot(arweave_sync_peer:goodput_improved(100.1, 100.0)),
    Control0 = #cap_control{cap = 100},
    Control1 = arweave_sync_peer:evolve_cap_control(100.0, 1000, 0.0, true, Control0),
    ...
    Control2 = arweave_sync_peer:evolve_cap_control(105.0, 1000, 0.0, true, Control1),
    ...
    Control3 = arweave_sync_peer:evolve_cap_control(105.0, 1000, 0.0, true, Control2),
    ?assertEqual(124, Control3#cap_control.cap),
    ?assertEqual(1, Control3#cap_control.observations),
    Control4 = arweave_sync_peer:evolve_cap_control(105.0, 1000, 0.0, true, Control3),
    ?assertEqual(132, Control4#cap_control.cap),
    ?assertEqual(2, Control4#cap_control.observations),
    Control5 = arweave_sync_peer:evolve_cap_control(105.0, 1000, 0.0, true, Control4),
    ?assertEqual(140, Control5#cap_control.cap),
    ?assertEqual(3, Control5#cap_control.observations),
    Control6 = arweave_sync_peer:evolve_cap_control(105.0, 1000, 0.0, true, Control5),
    ...
    ?assertEqual(
        Control6,
        arweave_sync_peer:evolve_cap_control(105.0, 0, 0.0, true, Control6)
    ),
    ?assertEqual(
        Control6,
        arweave_sync_peer:evolve_cap_control(105.0, 1000, 0.0, false, Control6)
    ),
    ...
    PressureCut = arweave_sync_peer:evolve_cap_control(
        110.0,
        1000,
        0.1,
        true,
    ...
    FullCut = arweave_sync_peer:evolve_cap_control(0.0, 0, 1.0, true, Control0),
    ?assertEqual(50, FullCut#cap_control.cap),
    MinimumCut = arweave_sync_peer:evolve_cap_control(
        0.0,
        0,
        1.0,
        true,
    ...
    {FastRejectPressure, 250} = arweave_sync_peer:failure_pressure(FastRejectTiming),
    FastRejectControl = arweave_sync_peer:evolve_cap_control(
...
small_failure_pressure_preserves_concurrency(_Config) ->
    ...
    {Pressure, 4000} = arweave_sync_peer:failure_pressure(#fetch_timing{
        productive_ms = 19 * 4000,
        client_error_ms = 4000
    }),
    SmallCut = arweave_sync_peer:evolve_cap_control(100.0, 19 * 4000, Pressure, true, Control),
    ...
    LargerCut = arweave_sync_peer:evolve_cap_control(100.0, 1000, 0.1, true, Control),
    ?assertEqual(7, LargerCut#cap_control.cap),
    %% An all-failure interval must still halve this small cap.
    FullCut = arweave_sync_peer:evolve_cap_control(0.0, 0, 1.0, true, Control),
...
goodput_backoff_settles_before_retry(_Config) ->
    ...
    Control1 = arweave_sync_peer:evolve_cap_control(90.0, 1000, 0.0, true, Control0),
    ?assertEqual(129, Control1#cap_control.cap),
    Control2 = arweave_sync_peer:evolve_cap_control(90.0, 1000, 0.0, true, Control1),
    ?assertEqual(137, Control2#cap_control.cap),
    Control3 = arweave_sync_peer:evolve_cap_control(90.0, 1000, 0.0, true, Control2),
    ?assertEqual(145, Control3#cap_control.cap),
    Control4 = arweave_sync_peer:evolve_cap_control(90.0, 1000, 0.0, true, Control3),
    ...
    SettlingControl = lists:foldl(
        fun(_, Control) ->
            arweave_sync_peer:evolve_cap_control(90.0, 1000, 0.0, true, Control)
        end,
        Control4,
        lists:seq(1, 3)
    ),
    ?assertEqual(105, SettlingControl#cap_control.cap),
    ControlAfterSettle = arweave_sync_peer:evolve_cap_control(
...
recompute(_Config) ->
    ...
    {Caps0, QueueMaxLengths0, S0} = arweave_sync_peer:recompute(
    ...
    {Caps1, _QueueMaxLengths1, S1} = arweave_sync_peer:recompute(
        [A, B],
        G,
    ...
    {Caps2, QueueMaxLengths2, S2} = arweave_sync_peer:recompute(
    ...
    {Caps3, _QueueMaxLengths3, _S3} = arweave_sync_peer:recompute(
    ...
    {Caps4, QueueMaxLengths4, _} = arweave_sync_peer:recompute(
        [A],
        #{A => 5242.88},
        #{A => Productive},
        No,
    ...
    {Caps5, QueueMaxLengths5, _} = arweave_sync_peer:recompute([A], #{}, #{}, No, #{}),
...
worker_time_pressure_isolated(_Config) ->
    ...
    {Caps1, _QueueMaxLengths1, State1} = arweave_sync_peer:recompute(
        [Healthy, TimedOut],
        Goodput,
        Timings,
        Yes,
    ...
    {Caps2, _QueueMaxLengths2, _State2} = arweave_sync_peer:recompute(
        [TimedOut],
        Goodput,
        Recovery,
        Yes,
...
delivery_caps(Peer, ChunksPerTick, RTTMs, TickMs) ->
    ...
    {_State, _Now, Caps} = lists:foldl(
        fun(Chunks, {State, Now, Acc}) ->
            ...
            State2 = arweave_sync_peer:tick([Peer], #{}, #{Peer => true}, Now2, State1),
