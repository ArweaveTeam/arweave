...
all() ->
    [
        ...
        test_runtime_download_rate_pause_and_resume,
...
init_per_suite(Config) ->
    {ok, SyncApps} = application:ensure_all_started(arweave_sync),
    {ok, EntropyApps} = application:ensure_all_started(arweave_entropy),
    [{started_apps, SyncApps ++ EntropyApps} | Config].
...
init_per_testcase(Case, Config) ->
    ct:timetrap({seconds, testcase_timeout(Case)}),
    ?assertEqual(undefined, whereis(ar_sup)),
    Snapshot = arweave_config:internal_snapshot(),
    ok = arweave_sync_sim:setup_fixture(),
    ?assertEqual(undefined, ets:info(ar_data_sync_state)),
    [{config_snapshot, Snapshot} | Config].

end_per_testcase(_Case, Config) ->
    arweave_sync_sim:teardown_fixture(),
...
test_runtime_download_rate_pause_and_resume(_Config) ->
    %% A forty-chunk/s peer has headroom above every tested download limit.
    arweave_sync_sim:start_sim(#sim_world{
        peers = #{?PEER_UNLIMITED => #sim_peer{max_serve_cps = 40}},
        node_config = #{[sync, max_download_rate] => 0}
    }),
    %% Exercise the runtime option validator, not just test config loading.
    ok = arweave_config:runtime(),
    Scheduler = whereis(arweave_sync_scheduler),
    Sweepers = [
        whereis(arweave_sync_store_sweeper:name(StoreID))
     || StoreID <- arweave_sim:store_ids()
    ],
    ?assert(is_pid(Scheduler)),
    ?assert(lists:all(fun is_pid/1, Sweepers)),
    %% Three seconds cover repeated sweep attempts while initially paused.
    InitiallyPaused = arweave_sync_sim:run_for(3),
    ?assertEqual(
        0,
        arweave_sync_sim:metric(
            {served_by_peer, ?PEER_UNLIMITED}, InitiallyPaused
        )
    ),
    ?assertEqual(
        0,
        arweave_sync_sim:metric(
            {chunk_interval_requests_by_peer, ?PEER_UNLIMITED}, InitiallyPaused
        )
    ),
    lists:foreach(
        fun(CPS) ->
            ok = arweave_config:set(
                [sync, max_download_rate], CPS * ?DATA_CHUNK_SIZE
            ),
            %% Ten seconds allow initial metadata discovery and prior rate debt to
            %% settle; five measured seconds span several refills at each rate.
            arweave_sync_sim:run_for(10),
            Measurement = arweave_sync_sim:run_for(5),
            Fetched = arweave_sync_sim:metric(
                {served_by_peer, ?PEER_UNLIMITED}, Measurement
            ),
            ?assert(Fetched > 0),
            ?assert(
                arweave_sync_sim:metric(chunks_stored_total, Measurement) > 0
            ),
            %% Five seconds plus a full one-second bucket and one task of overdraft.
            ?assert(Fetched =< CPS * (5 + 1) + 1)
        end,
        [4, 2, 8]
    ),
    ok = arweave_config:set([sync, max_download_rate], 0),
    %% Allow already-started fetches and writes to finish before measuring pause.
    arweave_sync_sim:run_for(2),
    Paused = arweave_sync_sim:run_for(3),
    ?assertEqual(
        0,
        arweave_sync_sim:metric(
            {served_by_peer, ?PEER_UNLIMITED}, Paused
        )
    ),
    ?assertEqual(
        0,
        arweave_sync_sim:metric(
            {chunk_interval_requests_by_peer, ?PEER_UNLIMITED}, Paused
        )
    ),
    ok = arweave_config:set([sync, max_download_rate], infinity),
    Resumed = arweave_sync_sim:run_for(5),
    ?assert(arweave_sync_sim:metric(chunks_stored_total, Resumed) > 0),
    ?assertEqual(Scheduler, whereis(arweave_sync_scheduler)),
    ?assertEqual(Sweepers, [
        whereis(arweave_sync_store_sweeper:name(StoreID))
     || StoreID <- arweave_sim:store_ids()
    ]).
...
test_steady_state_no_monopoly(_Config) ->
...
test_timeout_resilience(_Config) ->
    arweave_config:internal_with_test_config(fun() ->
        HealthyCPS = 25,
        LimitedCPS = 5,
        PhaseTicks = 10,
                    ...
                    Index ->
                        Peer#sim_peer{
                            failure_policy = fun(Tick, _RequestSequence) ->
                                case
                                    ((Tick div PhaseTicks) rem 5) =:= Index - 1 andalso
                                        (Tick rem PhaseTicks) < OutageTicks
                                of
                                    true -> timeout;
                                    false -> none
                                end
        ...
        %% Settle two complete outage rotations before measuring steady state.
        %% Thirty ticks left the last peers' first outages in the measured
        %% window, mixing initial cap adaptation into the recovery contract.
        arweave_sync_sim:run_for(2 * RotationTicks),
        Measurement = arweave_sync_sim:run_for(RotationTicks),
        CpsByPeer = arweave_sync_sim:metric(cps_by_peer, Measurement),
        ServedCPS = lists:sum(maps:values(CpsByPeer)),
        TimeoutsByPeer = arweave_sync_sim:metric(timed_out_by_peer, Measurement),
        ...
        %% The healthy peer is always available. Each limited peer serves 48 of
        %% the 50 measured seconds, so 25 + 5 * 5 * 48 / 50s = 49 chunks/s.
        ExpectedAvailableCPS =
            HealthyCPS +
                length(LimitedPeers) * LimitedCPS *
                    (RotationTicks - OutageTicks) / RotationTicks,
        assert_at_least(
            arweave_sync_sim:metric(stored_cps, Measurement),
...
test_rate_limit_settles_and_recovers(_Config) ->
    arweave_config:internal_with_test_config(fun() ->
        HighCps = 400,
        LimitedCps = 80,
        ...
        LimitedMeasurement = arweave_sync_sim:run_for(20),
        LimitedServed = arweave_sync_sim:metric(
            {served_by_peer, ?PEER_LIMITED_1}, LimitedMeasurement
        ),
        LimitedRejected = arweave_sync_sim:metric(
...
test_recurring_rate_limit_preserves_throughput(_Config) ->
    arweave_config:internal_with_test_config(fun() ->
        MaxServeCps = 2,
        RejectEvery = 8,
...
test_recurring_client_errors_preserve_throughput(_Config) ->
    arweave_config:internal_with_test_config(fun() ->
        MaxServeCPS = 2,
        ErrorEvery = 40,
...
test_single_peer_saturation(_Config) ->
    arweave_config:internal_with_test_config(fun() ->
        MaxServeCps = 100,
...
test_single_throttled_peer_keeps_progressing(_Config) ->
    arweave_config:internal_with_test_config(fun() ->
        MaxServeCps = 100,
        Peers = #{
...
test_peer_capacity_and_latency_growth(_Config) ->
    arweave_config:internal_with_test_config(fun() ->
        InitialServeCPS = 8,
        ...
        arweave_sync_sim:run_for(4),
        CapacityMeasurement = arweave_sync_sim:run_for(2),
        ...
        arweave_sync_sim:run_for(8),
        LatencyMeasurement = arweave_sync_sim:run_for(2),
...
test_client_error_recovery_with_http_headroom(_Config) ->
    arweave_config:internal_with_test_config(fun() ->
        MaxServeCps = 100,
        ...
        ForcedMeasurement = arweave_sync_sim:run_for(12),
        assert_greater_than(
            arweave_sync_sim:metric(
        ...
        Served = arweave_sync_sim:metric(
            {served_by_peer, ?PEER_UNLIMITED}, Measurement
        ),
        assert_metric_utilization(stored_cps, MaxServeCps, Measurement),
...
test_fast_peer_turns_slow(_Config) ->
    arweave_config:internal_with_test_config(fun() ->
        InitialFastCPS = 300,
        SiblingCPS = 60,
...
%%   longer than normal scheduler adaptation periods. After recovery, the smaller
%%   peer queues requests at its serving ceiling without injected failures or 429s.
%%
%% Timeline:
...
test_starved_peer_recovers(_Config) ->
    arweave_config:internal_with_test_config(fun() ->
        HealthyCPS = 200,
        RecoveredCps = 40,
        Peers = #{
            ...
            ?PEER_TIMEOUT => #sim_peer{
                ...
                %% Isolate timeout recovery from recurring 429 backpressure,
                %% covered by test_recurring_rate_limit_preserves_throughput.
        ...
        OutageMeasurement = arweave_sync_sim:run_for(20),
        Timeouts = arweave_sync_sim:metric(
        ...
        StarvedCps = arweave_sync_sim:metric(
            {cps_by_peer, ?PEER_TIMEOUT}, arweave_sync_sim:run_for(19)
        ),
        assert_less_than(
            StarvedCps,
            1,
        ...
        ?assertEqual(
            0,
            arweave_sync_sim:metric(
                {rejected_by_peer, ?PEER_TIMEOUT}, Measurement
            )
        ),
...
test_slow_disk_write_recovery(_Config) ->
    arweave_config:internal_with_test_config(fun() ->
        Peers = #{
            ?PEER_UNLIMITED => #sim_peer{max_serve_cps = 250},
            ?PEER_LIMITED_1 => #sim_peer{max_serve_cps = 60, limited = true},
        ...
        World = #sim_world{
            peers = Peers,
            %% 756 MiB / (3 * 256 KiB) gives capacity for 1008 chunks, evenly
            %% divided into six 168-chunk store shares. Stalled stores can
            %% therefore fill the global chunk cache.
            node_config = #{[packing, cache_size] => 756}
        },
        arweave_sync_sim:start_sim(World),
        ...
        FrozenMeasurement = arweave_sync_sim:run_for(5),
        ?assert((arweave_sync_deps:chunk_cache()):is_full()),
        FrozenCps = lists:sum(
            maps:values(
                arweave_sync_sim:metric(cps_by_peer, FrozenMeasurement)
            )
        ),
        assert_at_most(FrozenCps, 0.05 * capacity(Peers)),
        arweave_sync_sim:update_world(World),
        %% Five ticks allow the delayed writes to complete and fetching to restart.
        arweave_sync_sim:run_for(5),
...
test_slow_stores_do_not_monopolize_cache(_Config) ->
    arweave_config:internal_with_test_config(fun() ->
        Peers = #{
            ?PEER_UNLIMITED => #sim_peer{max_serve_cps = 250},
            ?PEER_LIMITED_1 => #sim_peer{max_serve_cps = 60, limited = true},
        ...
        %% 1500 MiB resolves to a cache holding 2000 chunks: large enough to carry the
        ...
        arweave_sync_sim:start_sim(World),
        {SlowStores, _Healthy} = lists:split(4, arweave_sim:store_ids()),
        ...
        %% horizon.
        arweave_sync_sim:run_for(40),
        ?assertNot((arweave_sync_deps:chunk_cache()):is_full()),
...
test_shared_slow_device_isolation(_Config) ->
    arweave_config:internal_with_test_config(fun() ->
        ...
        World = #sim_world{
            ...
            %% A 1500 MiB shared cache holds 2000 chunks. Four old one-quarter
            %% claim allowances can fill it. The six stores
        ...
        arweave_sync_sim:update_world(StalledWorld),
        ...
        MeasurementSeconds = arweave_sync_sim:metric(
...
test_mixed_peer_classes_rate_limited(_Config) ->
    arweave_config:internal_with_test_config(fun() ->
        BudgetCps = 120,
        ...
        WarmupMeasurement = arweave_sync_sim:run_for(40),
        Measurement = arweave_sync_sim:run_for(20),
        CpsByPeer = arweave_sync_sim:metric(cps_by_peer, Measurement),
        ServedCPS = lists:sum(maps:values(CpsByPeer)),
        WarmupTimeouts = arweave_sync_sim:metric(
            {timed_out_by_peer, ?PEER_FLAKY}, WarmupMeasurement
        ),
        MeasurementTimeouts = arweave_sync_sim:metric(
            {timed_out_by_peer, ?PEER_FLAKY}, Measurement
        ),
        Timeouts = WarmupTimeouts + MeasurementTimeouts,
        WarmupHTTP429s = arweave_sync_sim:metric(
            {rejected_by_peer, ?PEER_LIMITED_1}, WarmupMeasurement
        ),
        MeasurementHTTP429s = arweave_sync_sim:metric(
            {rejected_by_peer, ?PEER_LIMITED_1}, Measurement
        ),
...
test_single_peer_store_spread(_Config) ->
    arweave_config:internal_with_test_config(fun() ->
        StoreCps = 4,
        ...
        MeasurementTicks = 20,
        Measurement = arweave_sync_sim:run_for(MeasurementTicks),
...
test_single_footprint_peer_store_spread(_Config) ->
    arweave_config:internal_with_test_config(fun() ->
...
%% - V2: Every store remains within the default 90% equal-share floor.
test_single_footprint_peer_store_bursts_preserve_throughput(_Config) ->
    arweave_config:internal_with_test_config(fun() ->
        arweave_sim:use_mainnet_replica_2_9_sizes(),
        StoreIDs = arweave_sim:store_ids(),
...
test_delayed_fetch_waves_do_not_amplify_store_backlog(_Config) ->
    arweave_config:internal_with_test_config(fun() ->
        arweave_sim:use_mainnet_replica_2_9_sizes(),
        arweave_sync_scheduler:override_tick_interval_ms(10_000),
        StoreCPS = 4,
        ...
        arweave_sync_sim:run_for(12),
        DelayedPeer = Peer#sim_peer{latency_ms = DelayedLatencyMS},
        DelayedWorld = World#sim_world{
            peers = Peers#{
                ?PEER_UNLIMITED := DelayedPeer
            }
        },
        arweave_sync_sim:update_world(DelayedWorld),
        ...
        arweave_sync_sim:run_for(160),
        SaturationMeasurement = arweave_sync_sim:run_for(40),
        ...
        arweave_sync_sim:update_world(TransitionWorld),
        ...
        arweave_sync_sim:update_world(PressureWorld),
        ...
        lists:foreach(
            fun(DelayedMeasurement) ->
                assert_at_least(
                    arweave_sync_sim:metric(stored_cps, DelayedMeasurement),
...
test_low_local_completion_observation_does_not_limit_recovery(_Config) ->
    arweave_config:internal_with_test_config(fun() ->
        StoreCPS = 5,
        AggregateStoreCPS = ?SIM_STORES * StoreCPS,
        BatchPeriod = 4,
        ...
        arweave_sync_sim:run_for(LowWarmupSeconds),
        LowMeasurement = arweave_sync_sim:run_for(LowMeasurementSeconds),
        ExpectedLowCPS = ?SIM_STORES * LowObservationBatchSize / BatchPeriod,
        LowStoredCPS = arweave_sync_sim:metric(stored_cps, LowMeasurement),
        assert_at_least(
            LowStoredCPS,
        ...
        Measurement = arweave_sync_sim:run_for(40),
        assert_chunks_spread_across_stores(Measurement),
        assert_metric_utilization(stored_cps, AggregateStoreCPS, Measurement)
    end).
...
test_link_saturation(_Config) ->
    arweave_config:internal_with_test_config(fun() ->
        ...
        HTTPInflightByPeer = arweave_sync_sim:metric(
...
test_footprint_contention_preserves_throughput(_Config) ->
    arweave_config:internal_with_test_config(fun() ->
        PeerCPS = 60,
...
test_slow_footprint_peer_preserves_healthy_capacity(_Config) ->
    arweave_config:internal_with_test_config(fun() ->
        use_footprint_size(16),
        ...
        Measurement = arweave_sync_sim:run_for(10),
        CpsByPeer = arweave_sync_sim:metric(cps_by_peer, Measurement),
        HealthyCPS = lists:sum([maps:get(Peer, CpsByPeer, 0) || Peer <- HealthyPeerIDs]),
...
test_footprint_claim_headroom_preserves_peer_breadth(_Config) ->
    arweave_config:internal_with_test_config(fun() ->
        ...
        TotalStored = arweave_sync_sim:metric(
...
test_byte_and_footprint_sources_share_footprint(_Config) ->
    arweave_config:internal_with_test_config(fun() ->
        arweave_sim:use_mainnet_replica_2_9_sizes(),
        ...
        arweave_sync_sim:start_sim(InitialWorld),
        %% Thirty ticks cover metadata discovery plus the sparse byte chunk and the
        %% footprint source's nine remaining chunks.
        InitialMeasurement = arweave_sync_sim:run_for(30),
        assert_at_least(
            arweave_sync_sim:metric(
                {served_by_peer, SingleChunkPeer}, InitialMeasurement
            ),
            1
        ),
        assert_at_least(
            arweave_sync_sim:metric(
                {served_by_peer, FootprintPeer}, InitialMeasurement
            ),
            9
        ),
        ?assertEqual(
            AdvertisedChunkCount,
            arweave_sync_sim:metric(chunks_stored_total, InitialMeasurement)
        )
    end).
...
test_unpromoted_footprint_work_does_not_block_store(_Config) ->
    arweave_config:internal_with_test_config(fun() ->
        arweave_sim:use_mainnet_replica_2_9_sizes(),
        TargetStore = first_store(),
        ...
        FullWorld = #sim_world{
            ...
            %% A 1500 MiB chunk cache holds 2000 lifecycle reservations across six stores,
        ...
        #sim_snapshot{chunk_interval_requests_by_peer = MetadataRequests} =
            arweave_sim:snapshot(),
        assert_greater_than(
            maps:get(SlowFootprintPeer, MetadataRequests),
            0,
            #{
        ...
        {[CachedPeerRange], ok} = arweave_sync_discovery:cached_peer_ranges(
            TargetStore,
            [SlowFootprintPeer],
            StoreStart,
            StoreStart,
        ...
        DurationSeconds = arweave_sync_sim:metric(
            duration_seconds, Measurement
        ),
...
test_sparse_footprint_sources_preserve_throughput(_Config) ->
    arweave_config:internal_with_test_config(fun() ->
        arweave_sim:use_mainnet_replica_2_9_sizes(),
...
test_pending_chunk_intervals_do_not_stall_store(_Config) ->
    arweave_config:internal_with_test_config(fun() ->
        arweave_sim:use_mainnet_replica_2_9_sizes(),
        TargetStore = first_store(),
        StoreRange = first_store_range(),
...
test_repeated_empty_footprint_gaps_preserve_throughput(_Config) ->
    arweave_config:internal_with_test_config(fun() ->
        Peer = {10, 6, 6, 1, 1984},
        PeerCPS = 2,
...
%%   pending.
test_ready_footprint_peer_progresses_while_metadata_pending(_Config) ->
    arweave_config:internal_with_test_config(fun() ->
        HealthyPeer = {10, 7, 0, 1, 1984},
        SlowPeers = [{10, 7, 0, ID, 1984} || ID <- lists:seq(2, 5)],
        HealthyPeerCPS = 100,
        HealthyPeerSpec = #sim_peer{
            max_serve_cps = HealthyPeerCPS,
            sync_kinds = [footprint]
        },
        ...
        WarmupMeasurement = arweave_sync_sim:run_for(30),
        lists:foreach(
            fun(SlowPeer) ->
...
test_dual_advertisement_uses_byte_capacity(_Config) ->
    arweave_config:internal_with_test_config(fun() ->
        ...
        arweave_sim:use_mainnet_replica_2_9_sizes(),
        arweave_sync_cursor:override_query_range_step_size(QueryStep),
        arweave_sync_sim:start_sim(#sim_world{
            peers = Peers,
            ...
            %% 1500 MiB total sync budget resolves to a 2000-chunk cache, enough for
...
test_footprint_gossip_share_collapse(_Config) ->
    arweave_config:internal_with_test_config(fun() ->
        Peers = maps:merge(
            #{
                ?PEER_UNLIMITED => #sim_peer{
...
test_metadata_scarce_store_spread(_Config) ->
    arweave_config:internal_with_test_config(fun() ->
        ChunksPerRange = 64,
        MetadataLatencySeconds = 1,
        arweave_sync_cursor:override_query_range_step_size(
        ...
        PeerIntervals = [
         ...
         || {_StoreID, StoreRange} <- arweave_sim:store_ranges()
        ],
        Peers = #{
            ?PEER_UNLIMITED => #sim_peer{
                max_serve_cps = 400,
...
%% - V2: Stored throughput reaches 95% of aggregate store write capacity.
%% - V3: Every store remains within the default 90% equal-share floor.
test_discovery_backlog_preserves_store_throughput(_Config) ->
    arweave_config:internal_with_test_config(fun() ->
        ...
        %% 128 * 64 = 8192 chunks, well above the 120 seconds * 32 chunks/s
        ...
        arweave_sync_sim:start_sim(#sim_world{
            ...
            store_write_cps = StoreCPS,
            node_config = #{
                %% 117964 reservations hold over 600 seconds at the aggregate 192
        ...
        %% Allow two 40-tick settling periods: peer allocation settles first,
        %% then the mixed metadata backlog supplies the later stores. Measuring
        %% after only the first period can still include startup cache misses.
        WarmupMeasurement = arweave_sync_sim:run_for(80),
        MetadataRequests = lists:sum(
            maps:values(
                arweave_sync_sim:metric(
...
test_disjoint_peer_holdings(_Config) ->
    arweave_config:internal_with_test_config(fun() ->
        StoreRanges = arweave_sim:store_ranges(),
        PeerCps = 100,
        Peers = maps:from_list([
...
test_stalled_frontier_peer_does_not_hide_later_capacity(_Config) ->
    arweave_config:internal_with_test_config(fun() ->
        StoreRanges = arweave_sim:store_ranges(),
        ...
        BlockerMeasurement = arweave_sync_sim:run_for(20),
...
%% @doc The simulator derives chunk capacity from the node's MiB cache setting.
test_node_config_drives_cache_capacity(_Config) ->
    %% Each lifecycle reserves three 256 KiB representations.
    CacheMiB = 1000,
    ExpectedChunkCacheLimit = (1000 * ?MiB) div (3 * ?DATA_CHUNK_SIZE),
    arweave_sync_sim:start_sim(#sim_world{
        node_config = #{
            [packing, cache_size] => CacheMiB
        }
    }),
...
single_chunk_interval(StoreStart, ChunkIndex) ->
    ChunkEnd = arweave_constants:get_chunk_padded_offset(
...
useful_bucket_tail_footprint_offsets(
    StoreStart,
    Count,
    FootprintSize,
    NetworkBucketSize,
    ...
    StoreFirstOffset = arweave_storage:get_footprint_offset(FirstChunkEnd),
...
use_footprint_size(ChunkCount) ->
    ...
    arweave_constants:internal_override_replica_2_9_entropy_size(EntropySize).
...
assert_chunks_spread_across_stores(Measurement, MinimumFairShareFraction) ->
    ChunksStoredByStore = arweave_sync_sim:metric(
        chunks_stored_by_store, Measurement
    ),
    TotalChunksStored = arweave_sync_sim:metric(chunks_stored_total, Measurement),
    assert_greater_than(
        TotalChunksStored,
        0,
...
assert_metric_utilization(Metric, ExpectedValue, Measurement) ->
    ActualValue = arweave_sync_sim:metric(Metric, Measurement),
...
assert_rate_at_least(RateMetric, Peer, ExpectedRate, Measurement) ->
    RatesByPeer = arweave_sync_sim:metric(RateMetric, Measurement),
...
non_serving_peers(Count) ->
    maps:from_list([
        {{10, 99, N div 250, N rem 250, 1984}, #sim_peer{sync_kinds = []}}
     || N <- lists:seq(1, Count)
    ]).
...
testcase_timeout(test_client_error_recovery_with_http_headroom) -> 400;
testcase_timeout(test_fast_peer_turns_slow) -> 300;
testcase_timeout(test_starved_peer_recovers) -> 300;
testcase_timeout(test_slow_disk_write_recovery) -> 600;
testcase_timeout(test_slow_stores_do_not_monopolize_cache) -> 600;
testcase_timeout(test_shared_slow_device_isolation) -> 600;
...
testcase_timeout(test_link_saturation) -> 400;
testcase_timeout(test_runtime_download_rate_pause_and_resume) -> 120;
testcase_timeout(test_footprint_contention_preserves_throughput) -> 900;
testcase_timeout(test_slow_footprint_peer_preserves_healthy_capacity) -> 500;
testcase_timeout(test_footprint_claim_headroom_preserves_peer_breadth) -> 500;
testcase_timeout(test_byte_and_footprint_sources_share_footprint) -> 500;
testcase_timeout(test_unpromoted_footprint_work_does_not_block_store) -> 500;
testcase_timeout(test_sparse_footprint_sources_preserve_throughput) -> 700;
testcase_timeout(test_pending_chunk_intervals_do_not_stall_store) -> 300;
testcase_timeout(test_repeated_empty_footprint_gaps_preserve_throughput) -> 400;
testcase_timeout(test_ready_footprint_peer_progresses_while_metadata_pending) -> 400;
testcase_timeout(test_dual_advertisement_uses_byte_capacity) -> 300;
testcase_timeout(test_footprint_gossip_share_collapse) -> 500;
testcase_timeout(test_metadata_scarce_store_spread) -> 400;
