...
capacity_limits_are_chunk_granular(_Config) ->
    ...
    ?assertEqual(25, arweave_sync_store:do_cache_limit(#store_state{}, 1)),
    %% Five seconds at 100 chunks/s is 500 chunks.
    ?assertEqual(
        500,
        arweave_sync_store:claim_limit(#store_state{drain_rate = 100})
    ),
    %% Five seconds at 105 chunks/s is 525 chunks without footprint rounding.
    ?assertEqual(
        525,
        arweave_sync_store:claim_limit(#store_state{drain_rate = 105})
    ),
    %% Five seconds at fifteen chunks/s is a seventy-five-chunk cache limit.
    ?assertEqual(
        75,
        arweave_sync_store:do_cache_limit(#store_state{drain_rate = 15}, 1)
    ),
    %% Cached and fetching stages retain eleven seconds, or 165 chunks.
    ?assertEqual(
        165,
        arweave_sync_store:do_pipeline_limit(#store_state{drain_rate = 15}, 1)
    ),
    %% Five seconds at one chunk/s is below the 25-chunk progress probe.
    ?assertEqual(
        25,
        arweave_sync_store:claim_limit(#store_state{drain_rate = 1})
    ).
...
fetching_tasks_do_not_consume_store_cache_limit(_Config) ->
    ...
    ?assert(arweave_sync_store:has_capacity(Dispatch)),
    ?assertNot(
        arweave_sync_store:has_capacity(Dispatch#store_dispatch{
...
dispatch_best_store_orders_by_load(_Config) ->
    ...
    {ok, store_a} = arweave_sync_store:best_store(Dispatches),
    {ok, store_b} = arweave_sync_store:best_store(maps:remove(store_a, Dispatches)),
...
stores_by_peer_includes_task_and_footprint_demand(_Config) ->
    ...
    Dispatches0 = arweave_sync_store:start_dispatch(Footprints, #{}, [], States),
    ...
    ?assertEqual(
        ...
        arweave_sync_store:stores_by_peer(Dispatches)
    ),
    BlockedDispatches = #{
    ...
    ?assertEqual(#{}, arweave_sync_store:stores_by_peer(BlockedDispatches)).
...
pop_work_returns_queued_task(_Config) ->
    ...
    Dispatch = arweave_sync_store:test_dispatch(State),
    ?assertMatch({#task{}, #{}}, arweave_sync_store:pop_work(store1, Dispatch)).
...
work_removed_from_a_pass_returns_in_the_next_pass(_Config) ->
    ...
    Dispatch0 = arweave_sync_store:test_dispatch(State2),
    {First, Dispatch1} = arweave_sync_store:pop_work(store1, Dispatch0),
    %% Leaving blocked work out of the dispatch-local queue exposes the next
    %% item without changing persistent queued work.
    {Second, _Dispatch2} = arweave_sync_store:pop_work(store1, Dispatch1),
    ...
    NewDispatch = arweave_sync_store:test_dispatch(State2),
    {First, _NewDispatch2} = arweave_sync_store:pop_work(store1, NewDispatch).
...
drain_rate_uses_tick_samples(_Config) ->
    ...
    State1 = arweave_sync_store:sample_drain_rate(
    ...
    State2 = arweave_sync_store:sample_drain_rate(
        1,
    ...
    State3 = arweave_sync_store:sample_drain_rate(
        0,
    ...
    IncreasedState = arweave_sync_store:sample_drain_rate(
        1,
    ...
    State4 = arweave_sync_store:sample_drain_rate(
    ...
    State5 = arweave_sync_store:sample_drain_rate(
        1,
