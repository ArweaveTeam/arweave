...
-record(state, {
    file_index,
    store_id,
    %% some data we need pass to ar_entropy_storage
...
name(StoreID) ->
    list_to_atom("ar_chunk_storage_" ++ arweave_storage_module:label(StoreID)).
...
register_workers() ->
    ConfiguredWorkers = lists:map(
        fun(StorageModule) ->
            ...
            ChunkStorageName = arweave_storage_chunk_storage:name(StoreID),
            ?CHILD_WITH_ARGS(
                arweave_storage_chunk_storage,
                worker,
                ChunkStorageName,
                [ChunkStorageName, StoreID]
            )
        end,
        arweave_config:storage_modules()
    ),

    DefaultChunkStorageWorker = ?CHILD_WITH_ARGS(
        arweave_storage_chunk_storage,
        worker,
        ar_chunk_storage_default,
        [ar_chunk_storage_default, ?DEFAULT_MODULE]
    ),
...
is_storage_supported(Offset, ChunkSize, Packing) ->
    case Offset > arweave_constants:strict_data_split_threshold() of
        true ->
            %% All chunks above arweave_constants:strict_data_split_threshold() are placed in 256 KiB
            ...
            %% arweave_constants:strict_data_split_threshold() anyways), we put them to RocksDB.
...
put(PaddedOffset, Chunk, Packing, StoreID) ->
    ...
    arweave_metrics:histogram_observe(
        chunk_storage_put_duration_milliseconds,
...
do_put_call(PaddedOffset, Chunk, Packing, StoreID) ->
    ...
    case
            ...
            GenServerID, {put, PaddedOffset, Chunk, Packing}, 180_000
        )
    of
        ...
        {'EXIT', {timeout, {gen_server, call, _}}} ->
            ?LOG_ERROR([
                {event, gen_server_timeout_putting_chunk},
        ...
        {'EXIT', Reason} ->
            ...
            ?LOG_ERROR([
                {event, gen_server_exit_putting_chunk},
...
open_files(StoreID) ->
    ets:foldl(
        fun
            ({{Key, ID}, Filepath}, _) when ID == StoreID ->
...
get(Byte, StoreID) ->
    case
        arweave_storage:get_interval(
            Byte + 1, any_packing, {ar_chunk_storage, byte}, StoreID
        )
    of
        not_found ->
            not_found;
...
get(Byte, IntervalStart, StoreID) ->
    ...
    arweave_metrics:histogram_observe(
        chunk_storage_read_duration_milliseconds,
        [arweave_storage_module:label(StoreID)],
        erlang:monotonic_time() - StartTime
    ),
    case Result of
...
get_range(Start, Size, StoreID) ->
    ...
    of
        {_End, IntervalStart} when Start + Size > IntervalStart ->
            ...
            LastChunkFileStart = get_chunk_file_start_by_start_offset(
                LastChunkStart
            ),
            ...
            case ChunkFileStart /= LastChunkFileStart of
                false ->
                    ...
                    get(
                        Start2, ChunkStart, ChunkFileStart, StoreID, ChunkCount
                    );
                true ->
                    SizeBeforeBorder =
                        ChunkFileStart + get_chunk_group_size() - ChunkStart,
                    ChunkCountBeforeBorder =
                        max(SizeBeforeBorder, ?DATA_CHUNK_SIZE) div
                            ?DATA_CHUNK_SIZE,
                    StartAfterBorder =
                        ChunkStart + ChunkCountBeforeBorder * ?DATA_CHUNK_SIZE,
                    SizeAfterBorder =
                        Size2 - ChunkCountBeforeBorder * ?DATA_CHUNK_SIZE +
                            (Start2 - ChunkStart),
                    get(
                        Start2,
                        ChunkStart,
    ...
    case DefragEnabled of
        ...
        true ->
            ...
            arweave_storage_deps:console(
                "Defragmentation threshold: ~B bytes.~n", [Threshold]
            ),
            Sizes = read_chunks_sizes(DataDir),
            Files = files_to_defrag(
                arweave_config:defrag_storage_modules(),
                DataDir,
                Threshold,
                Sizes
            ),
...
storage_module_path(DataDir, ?DEFAULT_MODULE) ->
    arweave_storage_module:path(DataDir, ?DEFAULT_MODULE);
storage_module_path(DataDir, StoreID) ->
    DiskDirName = arweave_storage_module:disk_dir_name(StoreID),
    arweave_storage_module:path(DataDir, DiskDirName).
...
get_chunk_bucket_start(Offset) ->
    PaddedEndOffset = arweave_constants:get_chunk_padded_offset(Offset),
    arweave_util:floor_int(
        max(0, PaddedEndOffset - ?DATA_CHUNK_SIZE), ?DATA_CHUNK_SIZE
    ).
...
%%
%% If Offset is before the arweave_constants:strict_data_split_threshold() we just return it because we don't
%% have any information about where chunks start or end.
get_chunk_seek_offset(Offset) ->
    case Offset > arweave_constants:strict_data_split_threshold() of
        true ->
            arweave_constants:get_padded_offset(
                Offset, arweave_constants:strict_data_split_threshold()
            ) -
                (?DATA_CHUNK_SIZE) +
                1;
        false ->
...
init(StoreID) ->
    ...
    EntropyContext = arweave_storage_entropy_storage:initialize_context(
        StoreID, arweave_storage_module:get_packing(StoreID)
    ),
...
warn_custom_chunk_group_size(StoreID) ->
    case
        StoreID == ?DEFAULT_MODULE andalso
            get_chunk_group_size() /= ?CHUNK_GROUP_SIZE
    of
        true ->
            ...
            WarningMessage =
                "WARNING: changing chunk_storage_file_size is not "
                "recommended and may cause errors if different sizes are used for the same "
                "chunk storage files.",
            arweave_storage_deps:console(WarningMessage),
...
handle_call({put, PaddedEndOffset, Chunk, Packing}, _From, State) when
    byte_size(Chunk) == ?DATA_CHUNK_SIZE;
    Packing == unpacked_padded,
    element(1, Chunk) == chunk_with_entropy,
    byte_size(element(2, Chunk)) == ?DATA_CHUNK_SIZE,
    byte_size(element(3, Chunk)) == ?DATA_CHUNK_SIZE
->
    #state{
        store_id = StoreID,
...
handle_call({delete, PaddedEndOffset}, _From, State) ->
    ...
    case
        arweave_storage:delete_sync_record(
            PaddedEndOffset, StartOffset, {ar_chunk_storage, byte}, StoreID
        )
    of
        ok ->
            case
                arweave_storage:delete_entropy_record(PaddedEndOffset, StoreID)
            of
                ok ->
...
handle_call(
    reset, _, #state{store_id = StoreID, file_index = FileIndex} = State
) ->
    maps:map(
        fun(_Key, Filepath) ->
    ...
    ok = arweave_storage:cut_sync_record(0, {ar_chunk_storage, byte}, StoreID),
    erlang:erase(),
    {reply, ok, State#state{file_index = #{}}};
handle_call(Request, _From, State) ->
    ?LOG_WARNING([
        {event, unhandled_call}, {module, ?MODULE}, {request, Request}
    ]),
    {reply, ok, State}.

handle_info({Ref, _Reply}, State) when is_reference(Ref) ->
    ?LOG_ERROR([
        {event, stale_gen_server_call_reply}, {ref, Ref}, {reply, _Reply}
    ]),
...
write_chunk(PaddedOffset, Chunk, FileIndex, StoreID) ->
    ...
    case get_handle_by_filepath(Filepath) of
        ...
        F ->
            write_chunk2(
                PaddedOffset, ChunkOffset, Chunk, Filepath, F, Position
            )
    end.
...
get_position_and_relative_chunk_offset(ChunkFileStart, Offset) ->
    ...
    get_position_and_relative_chunk_offset_by_start_offset(
        ChunkFileStart, BucketPickOffset
    ).
...
get_position_and_relative_chunk_offset_by_start_offset(
    ...
    ChunkOffset =
        case BucketPickOffset - BucketStart of
            0 ->
    ...
    Position =
        RelativeOffset + ?OFFSET_SIZE * (RelativeOffset div ?DATA_CHUNK_SIZE),
    {Position, ChunkOffset}.
...
delete_chunk(PaddedOffset, StoreID) ->
    ...
    case file:open(Filepath, [read, write, raw]) of
        {ok, F} ->
            ...
            arweave_storage_entropy_storage:acquire_semaphore(Filepath),
            try
                file:pwrite(F, Position, ZeroChunk)
            after
                arweave_storage_entropy_storage:release_semaphore(Filepath),
                file:close(F)
            end;
        {error, enoent} ->
            ok;
    ...
    ReadChunks =
        case erlang:get({cfile, {ChunkFileStart, StoreID}}) of
            undefined ->
                case
                    ets:lookup(
                        chunk_storage_file_index, {ChunkFileStart, StoreID}
                    )
                of
                    [] ->
                        [];
            ...
            File ->
                read_chunk2(
                    Byte, Start, ChunkFileStart, File, ChunkCount, StoreID
                )
        end,
    case arweave_storage_module:is_repack_in_place(StoreID) of
        ...
        false ->
            filter_by_sync_record(
                ReadChunks, Byte, Start, ChunkFileStart, StoreID, ChunkCount
            )
    end.
...
extract_end_offset_chunk_pairs(
    <<ChunkOffset:?OFFSET_BIT_SIZE, Chunk:?DATA_CHUNK_SIZE/binary,
        Rest/binary>>,
    BucketStart,
    Shift
) ->
...
extract_end_offset_chunk_pairs(<<>>, _BucketStart, _Shift) ->
    [];
extract_end_offset_chunk_pairs(
    <<ChunkOffset:?OFFSET_BIT_SIZE, Chunk/binary>>,
    BucketStart,
    Shift
) ->
    ?LOG_ERROR([
        {event, unexpected_chunk_data},
...
filter_by_sync_record(
    ...
    prometheus_histogram:observe_duration(
        chunk_storage_sync_record_check_duration_milliseconds,
        [ChunkCount],
        fun() ->
...
filter_by_sync_record(
    Chunks, _Intervals, _Byte, _Start, _ChunkFileStart, _StoreID, 1
) ->
    %% The code paths which query a single chunk have already implicitly checked that
    %% the chunk belongs to the sync_record. E.g. arweave_storage:get_chunk/2
    Chunks;
filter_by_sync_record(
    [], _Intervals, _Byte, _Start, _ChunkFileStart, _StoreID, _ChunkCount
) ->
    [];
filter_by_sync_record(
    [{PaddedEndOffset, Chunk} | Rest],
    Intervals,
    Byte,
    Start,
    ChunkFileStart,
...
list_files(StoreID) ->
    list_files(arweave_config:get([data_dir]), StoreID).
...
files_to_defrag(StorageModules, DataDir, ByteSizeThreshold, Sizes) ->
    AllFiles = lists:flatmap(
        fun(StorageModule) ->
            list_files(DataDir, arweave_storage_module:id(StorageModule))
        end,
        StorageModules
    ),
    lists:filter(
...
defrag_files([Filepath | Rest]) ->
    ...
    arweave_storage_deps:console("Defragmenting ~s...~n", [Filepath]),
    TmpFilepath = Filepath ++ ".tmp",
    DefragCmd = io_lib:format("rsync --sparse --quiet ~ts ~ts", [
    ...
    arweave_storage_deps:console("Defragmented ~s...~n", [Filepath]),
    defrag_files(Rest).
