...
%%% @doc Common Test suite for arweave_limiter_group.
%%%
%%% Pure helper testcases (expire, add_and_order, ...) run with no
%%% fixture. The stateful testcases each start their own limiter
%%% process (with a per-testcase configuration) in init_per_testcase
%%% and tear it down in end_per_testcase, mirroring the foreachx
%%% fixture of the original eunit module.
%%% @end
%%%===================================================================
...
-export([
         expire/1,
         add_and_order/1,
         cleanup_timestamps_map/1,
         build_headers_info_sliding/1,
         noproc/1,
         timeout/1,
         simple_sliding_happy/1,
         simple_leaky_happy_path/1,
         rate_limiter_rejected_due_concurrency/1,
         rejected_due_leaky_rate/1,
         both_exhausted/1,
         peer_cleanup/1,
         leaky_manual_reduction/1,
         leaky_manual_reduction_disabled/1
        ]).
