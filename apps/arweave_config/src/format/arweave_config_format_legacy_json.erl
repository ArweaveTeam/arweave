...
parse_options([{<<"block_gossip_peers">>, Peers} | _]) ->
	{error, {bad_type, peers, array}, Peers};
...
parse_options([{<<"local_peers">>, Peers} | _]) ->
	{error, {bad_type, local_peers, array}, Peers};

parse_options([{<<"sync_from_local_peers_only">>, true} | Rest]) ->
	_ = arweave_config:set([sync, local_peers_only], true),
	parse_options(Rest);
parse_options([{<<"sync_from_local_peers_only">>, false} | Rest]) ->
	_ = arweave_config:set([sync, local_peers_only], false),
	parse_options(Rest);
parse_options([{<<"sync_from_local_peers_only">>, Opt} | _]) ->
	{error, {bad_type, sync_from_local_peers_only, boolean}, Opt};

parse_options([{<<"start_from_latest_state">>, true} | Rest]) ->
	_ = arweave_config:set([join, start_from_latest_state], true),
	parse_options(Rest);
parse_options([{<<"start_from_latest_state">>, false} | Rest]) ->
	_ = arweave_config:set([join, start_from_latest_state], false),
	parse_options(Rest);
parse_options([{<<"start_from_latest_state">>, Opt} | _]) ->
	{error, {bad_type, start_from_latest_state, boolean}, Opt};

parse_options([{<<"start_from_state">>, Folder} | Rest]) when is_binary(Folder) ->
	_ = arweave_config:set([join, start_from_state], binary_to_list(Folder)),
	parse_options(Rest);
parse_options([{<<"start_from_state">>, Folder} | _]) ->
	{error, {bad_type, start_from_state, string}, Folder};

parse_options([{<<"start_from_block">>, H} | Rest]) when is_binary(H) ->
...
parse_options([{<<"start_from_block">>, Opt} | _]) ->
	{error, {bad_type, start_from_block, string}, Opt};

parse_options([{<<"start_from_block_index">>, true} | Rest]) ->
	_ = arweave_config:set([join, start_from_latest_state], true),
	parse_options(Rest);
parse_options([{<<"start_from_block_index">>, false} | Rest]) ->
	_ = arweave_config:set([join, start_from_latest_state], false),
	parse_options(Rest);
parse_options([{<<"start_from_block_index">>, Opt} | _]) ->
	{error, {bad_type, start_from_block_index, boolean}, Opt};

parse_options([{<<"mine">>, true} | Rest]) ->
	_ = arweave_config:set([mining, enabled], true),
	parse_options(Rest);
parse_options([{<<"mine">>, false} | Rest]) ->
	parse_options(Rest);
parse_options([{<<"mine">>, Opt} | _]) ->
	{error, {bad_type, mine, boolean}, Opt};

parse_options([{<<"verify">>, <<"purge">>} | Rest]) ->
	_ = arweave_config:set([verify, mode], purge),
	parse_options(Rest);
parse_options([{<<"verify">>, <<"log">>} | Rest]) ->
	_ = arweave_config:set([verify, mode], log),
	parse_options(Rest);
parse_options([{<<"verify">>, Opt} | _]) ->
	{error, bad_verify_mode, Opt};

parse_options([{<<"verify_samples">>, N} | Rest]) when is_integer(N) ->
	_ = arweave_config:set([verify, samples], N),
	parse_options(Rest);
parse_options([{<<"verify_samples">>, <<"all">>} | Rest]) ->
	_ = arweave_config:set([verify, samples], all),
	parse_options(Rest);
parse_options([{<<"verify_samples">>, Opt} | _]) ->
	{error, {bad_type, verify_samples, number}, Opt};

parse_options([{<<"vdf">>, Mode} | Rest]) ->
...
parse_options([{<<"port">>, Port} | Rest]) when is_integer(Port) ->
	_ = arweave_config:set([port], Port),
	parse_options(Rest);
parse_options([{<<"port">>, Port} | _]) ->
	{error, {bad_type, port, number}, Port};

parse_options([{<<"data_dir">>, DataDir} | Rest]) when is_binary(DataDir) ->
	_ = arweave_config:set([data_dir], binary_to_list(DataDir)),
	parse_options(Rest);
parse_options([{<<"data_dir">>, DataDir} | _]) ->
	{error, {bad_type, data_dir, string}, DataDir};

parse_options([{<<"log_dir">>, Dir} | Rest]) when is_binary(Dir) ->
	_ = arweave_config:set([log_dir], binary_to_list(Dir)),
	parse_options(Rest);
parse_options([{<<"log_dir">>, Dir} | _]) ->
	{error, {bad_type, log_dir, string}, Dir};

parse_options([{<<"storage_modules">>, L} | Rest]) when is_list(L) ->
	try
...
parse_options([{<<"storage_modules">>, Bin} | _]) ->
	{error, {bad_type, storage_modules, array}, Bin};

parse_options([{<<"repack_batch_size">>, N} | Rest]) when is_integer(N) ->
...
parse_options([{<<"polling">>, Frequency} | Rest]) when is_integer(Frequency) ->
	...
	parse_options(Rest);
parse_options([{<<"polling">>, Opt} | _]) ->
	{error, {bad_type, polling, number}, Opt};

parse_options([{<<"block_pollers">>, N} | Rest]) when is_integer(N) ->
	_ = arweave_config:set([gossip, block, pollers], N),
	parse_options(Rest);
parse_options([{<<"block_pollers">>, Opt} | _]) ->
	{error, {bad_type, block_pollers, number}, Opt};

parse_options([{<<"no_auto_join">>, true} | Rest]) ->
	_ = arweave_config:set([join, auto], false),
	parse_options(Rest);
parse_options([{<<"no_auto_join">>, false} | Rest]) ->
	parse_options(Rest);
parse_options([{<<"no_auto_join">>, Opt} | _]) ->
	{error, {bad_type, no_auto_join, boolean}, Opt};

parse_options([{<<"join_workers">>, N} | Rest]) when is_integer(N)->
	_ = arweave_config:set([join, workers], N),
	parse_options(Rest);
parse_options([{<<"join_workers">>, Opt} | _]) ->
	{error, {bad_type, join_workers, number}, Opt};

parse_options([{<<"packing_workers">>, N} | Rest]) when is_integer(N)->
	_ = arweave_config:set([packing, workers], N),
	parse_options(Rest);
parse_options([{<<"packing_workers">>, Opt} | _]) ->
	{error, {bad_type, packing_workers, number}, Opt};

parse_options([{<<"replica_2_9_workers">>, N} | Rest]) when is_integer(N)->
	_ = arweave_config:set([packing, entropy, workers], N),
	parse_options(Rest);
parse_options([{<<"replica_2_9_workers">>, Opt} | _]) ->
	{error, {bad_type, replica_2_9_workers, number}, Opt};

parse_options([{<<"disable_replica_2_9_device_limit">>, true} | Rest]) ->
	_ = arweave_config:set([disable_device_limit], true),
	parse_options(Rest);
parse_options([{<<"disable_replica_2_9_device_limit">>, false} | Rest]) ->
	parse_options(Rest);
parse_options([{<<"disable_replica_2_9_device_limit">>, Opt} | _]) ->
	{error, {bad_type, disable_replica_2_9_device_limit, boolean}, Opt};

parse_options([{<<"replica_2_9_entropy_cache_size_mb">>, N} | Rest]) when is_integer(N)->
	_ = arweave_config:set([packing, entropy, cache_size], N),
	parse_options(Rest);
parse_options([{<<"replica_2_9_entropy_cache_size_mb">>, Opt} | _]) ->
	{error, {bad_type, replica_2_9_entropy_cache_size_mb, number}, Opt};

parse_options([{<<"diff">>, Diff} | Rest]) when is_integer(Diff) ->
	_ = arweave_config:set([genesis, difficulty], Diff),
	parse_options(Rest);
parse_options([{<<"diff">>, Diff} | _]) ->
	{error, {bad_type, diff, number}, Diff};

parse_options([{<<"mining_addr">>, Addr} | Rest]) when is_binary(Addr) ->
...
parse_options([{<<"enable">>, Features} | _]) ->
	{error, {bad_type, enable, array}, Features};

parse_options([{<<"disable">>, Features} | Rest]) when is_list(Features) ->
	case safe_map(fun(Feature) -> binary_to_atom(Feature, latin1) end, Features) of
		{ok, FeatureAtoms} ->
			lists:foreach(
...
parse_options([{<<"disable">>, Features} | _]) ->
	{error, {bad_type, disable, array}, Features};

parse_options([{<<"webhooks">>, WebhookConfigs} | Rest]) when is_list(WebhookConfigs) ->
...
parse_options([{<<"webhooks">>, Webhooks} | _]) ->
...
parse_options([{<<"semaphores">>, Semaphores} | _]) ->
	{error, {bad_type, semaphores, object}, Semaphores};

parse_options([{<<"max_connections">>, MaxConnections} | Rest])
		when is_integer(MaxConnections), MaxConnections >= 1 ->
	_ = arweave_config:set([network, server, tcp, max_connections], MaxConnections),
	parse_options(Rest);

parse_options([{<<"disk_pool_data_root_expiration_time">>, D} | Rest])
		when is_integer(D) ->
	_ = arweave_config:set([disk_pool, data_root_expiration_time], D),
	parse_options(Rest);

parse_options([{<<"max_disk_pool_buffer_mb">>, D} | Rest]) when is_integer(D) ->
	_ = arweave_config:set([disk_pool, max_buffer_size], D),
	parse_options(Rest);

parse_options([{<<"max_disk_pool_data_root_buffer_mb">>, D} | Rest])
		when is_integer(D) ->
	_ = arweave_config:set([disk_pool, max_data_root_buffer_size], D),
	parse_options(Rest);

parse_options([{<<"max_duplicate_data_roots">>, <<"infinity">>} | Rest]) ->
	_ = arweave_config:set([gossip, data_roots, max_duplicates], infinity),
	parse_options(Rest);
parse_options([{<<"max_duplicate_data_roots">>, D} | Rest])
		when is_integer(D) ->
	_ = arweave_config:set([gossip, data_roots, max_duplicates], D),
	parse_options(Rest);

parse_options([{<<"disk_cache_size_mb">>, D} | Rest]) when is_integer(D) ->
	_ = arweave_config:set([gossip, header_cache_size], D),
	parse_options(Rest);

parse_options([{<<"max_nonce_limiter_validation_thread_count">>, D} | Rest])
		when is_integer(D) ->
	_ = arweave_config:set([vdf, max_validation_threads], D),
	parse_options(Rest);

parse_options([{<<"max_nonce_limiter_last_step_validation_thread_count">>, D} | Rest])
		when is_integer(D) ->
	_ = arweave_config:set([vdf, max_last_step_validation_threads], D),
	parse_options(Rest);

parse_options([{<<"vdf_server_trusted_peer">>, <<>>} | Rest]) ->
	parse_options(Rest);
parse_options([{<<"vdf_server_trusted_peer">>, Peer} | Rest]) ->
	add_vdf_server_trusted_peer(Peer),
	parse_options(Rest);

parse_options([{<<"vdf_server_trusted_peers">>, Peers} | Rest]) when is_list(Peers) ->
	lists:foreach(fun add_vdf_server_trusted_peer/1, Peers),
	parse_options(Rest);
parse_options([{<<"vdf_server_trusted_peers">>, Peers} | _]) ->
...
parse_options([{<<"debug">>, B} | Rest]) when is_boolean(B) ->
	_ = arweave_config:set([debug], B),
	parse_options(Rest);

parse_options([{<<"run_defragmentation">>, B} | Rest]) when is_boolean(B) ->
	_ = arweave_config:set([defrag, enabled], B),
	parse_options(Rest);

parse_options([{<<"defragmentation_trigger_threshold">>, D} | Rest])
		when is_integer(D) ->
	_ = arweave_config:set([defrag, threshold], D),
	parse_options(Rest);

parse_options([{<<"block_throttle_by_ip_interval">>, D} | Rest])
		when is_integer(D) ->
	_ = arweave_config:set([gossip, block, throttle_by_ip_interval], D),
	parse_options(Rest);

parse_options([{<<"block_throttle_by_solution_interval">>, D} | Rest])
		when is_integer(D) ->
	_ = arweave_config:set([gossip, block, throttle_by_solution_interval], D),
	parse_options(Rest);

parse_options([{<<"defragment_modules">>, L} | Rest]) when is_list(L) ->
	try
		DefragModules =
			lists:foldr(
				fun(Bin, Acc) ->
...
parse_options([{<<"network.socket.backend">>, Backend}|Rest]) ->
	case Backend of
		<<"inet">> ->
...
parse_options([{<<"http_client.http.closing_timeout">>, Timeout}|Rest]) ->
	case Timeout of
...
parse_options([{<<"http_client.http.keepalive">>, Timeout}|Rest]) ->
	case Timeout of
		<<"infinity">> ->
...
parse_options([{<<"http_client.tcp.delay_send">>, Delay}|Rest]) ->
	case Delay of
...
parse_options([{<<"http_client.tcp.keepalive">>, Keepalive}|Rest]) ->
	case Keepalive of
...
parse_options([{<<"http_client.tcp.linger">>, Linger}|Rest]) ->
	case Linger of
...
parse_options([{<<"http_client.tcp.linger_timeout">>, Timeout}|Rest]) ->
	case Timeout of
		_ when is_integer(Timeout), Timeout >= 0 ->
...
parse_options([{<<"http_client.tcp.nodelay">>, Nodelay}|Rest]) ->
	case Nodelay of
...
parse_options([{<<"http_client.tcp.send_timeout_close">>, Value}|Rest]) ->
	case Value of
...
parse_options([{<<"http_client.tcp.send_timeout">>, Timeout}|Rest]) ->
	case Timeout of
		_ when is_integer(Timeout), Timeout >= 0 ->
...
parse_options([{<<"http_api.http.active_n">>, Active}|Rest]) ->
	case Active of
...
parse_options([{<<"http_api.http.inactivity_timeout">>, Timeout}|Rest]) ->
	case Timeout of
		_ when is_integer(Timeout), Timeout >= 0 ->
...
parse_options([{<<"http_api.http.linger_timeout">>, Timeout}|Rest]) ->
	case Timeout of
		_ when is_integer(Timeout), Timeout >= 0 ->
...
parse_options([{<<"http_api.http.request_timeout">>, Timeout}|Rest]) ->
	case Timeout of
		_ when is_integer(Timeout), Timeout >= 0 ->
...
parse_options([{<<"http_api.tcp.backlog">>, Backlog}|Rest]) ->
	case Backlog of
...
parse_options([{<<"http_api.tcp.delay_send">>, Delay}|Rest]) ->
	case Delay of
		_ when is_boolean(Delay) ->
...
parse_options([{<<"http_api.tcp.keepalive">>, Keepalive}|Rest]) ->
	case Keepalive of
		_ when is_boolean(Keepalive) ->
...
parse_options([{<<"http_api.tcp.linger">>, Linger}|Rest]) ->
	case Linger of
		_ when is_boolean(Linger) ->
...
parse_options([{<<"http_api.tcp.linger_timeout">>, Timeout}|Rest]) ->
	case Timeout of
		_ when is_integer(Timeout), Timeout >= 0 ->
...
parse_options([{<<"http_api.tcp.listener_shutdown">>, Shutdown}|Rest]) ->
	case Shutdown of
		"brutal_kill" ->
...
parse_options([{<<"http_api.tcp.nodelay">>, Nodelay}|Rest]) ->
	case Nodelay of
		_ when is_boolean(Nodelay) ->
...
parse_options([{<<"http_api.tcp.num_acceptors">>, Acceptors}|Rest]) ->
	case Acceptors of
...
parse_options([{<<"http_api.tcp.send_timeout_close">>, Value}|Rest]) ->
	case Value of
		_ when is_boolean(Value) ->
...
parse_options([{<<"http_api.tcp.send_timeout">>, Timeout}|Rest]) ->
	case Timeout of
		_ when is_integer(Timeout), Timeout >= 0 ->
