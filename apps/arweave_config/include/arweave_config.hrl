...
%% Tests run unevenly with unrealistically high request rates against the
%% data_sync_record group, so the test build uses a much higher leaky-bucket
%% limit than production.
-ifdef(AR_TEST).
-define(DEFAULT_HTTP_API_LIMITER_DATA_SYNC_RECORD_LEAKY_LIMIT, 1000).
-else.
