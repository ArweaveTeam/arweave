...
static ERL_NIF_TERM rsp_mix_entropy_crc32_test_nif(
		ErlNifEnv* envPtr, int argc, const ERL_NIF_TERM argv[]) {
	...
	rsp_mix_entropy_crc32(inEntropyBin.data, outEntropyData, entropySize);
...
static ERL_NIF_TERM rsp_mix_entropy_far_test_nif(
	...
	rsp_mix_entropy_far(inEntropyBin.data, outEntropyData, entropySize,
			jumpSize, blockSize);
...
static ErlNifFunc rxsquared_funcs[] = {
	...
	{"rsp_mix_entropy_crc32_test_nif", 1,
		rsp_mix_entropy_crc32_test_nif, ERL_NIF_DIRTY_JOB_CPU_BOUND},
