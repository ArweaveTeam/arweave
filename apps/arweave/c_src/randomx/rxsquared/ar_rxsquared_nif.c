...
static ERL_NIF_TERM rsp_fused_entropy_nif(ErlNifEnv* envPtr, int argc, const ERL_NIF_TERM argv[]) {
	...
	if (!success) {
		...
		free(vmList);
		return error_tuple(envPtr, "rsp_fused_entropy_failed");
	}
...
static ERL_NIF_TERM rsp_fused_entropy_low_latency_nif(ErlNifEnv* envPtr, int argc, const ERL_NIF_TERM argv[]) {
	if (argc != 10) {
		return enif_make_badarg(envPtr);
	}
	...
	// 7. Call the pure C++ function that does the heavy logic and returns bool
	int success = rsp_fused_entropy_low_latency(
		vmList,
		scratchpadSize,
		replicaEntropySubChunkCount,
		compositePackingSubChunkSize,
...
static ErlNifFunc rxsquared_funcs[] = {
	...
	{"rsp_fused_entropy_low_latency_nif", 10,
		rsp_fused_entropy_low_latency_nif, ERL_NIF_DIRTY_JOB_CPU_BOUND},
