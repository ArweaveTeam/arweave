...
%% The speed in chunks/s of moving the fork 2.6 packing threshold.
-ifdef(DEBUG).
...
-record(block, {
	...
	%% The offset on the weave separting the data which has to be packed for mining after the
	%% fork 2.6 from the data which is packed for 2.5. It is set to the weave_size of the 50th
	%% previous block at the hard fork block and moves down at a speed of
	%% ?PACKING_2_6_THRESHOLD_CHUNKS_PER_SECOND chunks/s. The motivation behind the threshold
	%% is a smooth transition to the new algorithm - big miners who might not want to adopt
	%% the new algorithm are still incentivized to upgrade and stay in the network for some
	%% time.
...
-record(tx, {
	...
	%% The type of signature this transaction was signed with.
