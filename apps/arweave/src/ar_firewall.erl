...
scan_tx(TX) ->
	...
	receive
		{scanned_tx, Ref, Response} ->
			Response
	after 30000 ->
		?LOG_ERROR([{ar_firewall, scan_tx_timeout}, {tx, ar_util:encode(TX#tx.id)}]),
		reject
	end.
...
scan_and_clean_disk() ->
	...
	spawn(
		fun() ->
			...
			?LOG_INFO([{ar_firewall, disk_scan_complete}]),
...
scan_transaction(TX, TXBlacklist, ContentSigs) ->
	case ar_tx_blacklist:is_blacklisted(TX, TXBlacklist) of
		true ->
			?LOG_INFO([{ar_firewall, reject_tx}, {tx, ar_util:encode(TX#tx.id)}, tx_is_blacklisted]),
			reject;
		false ->
			case ContentSigs of
				{[], no_pattern} ->
