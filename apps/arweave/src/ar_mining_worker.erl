...
handle_cast(?MSG_HANDLE_TASK, #state{ task_queue = Q } = State) ->
    case gb_sets:is_empty(Q) of
        ...
        _ ->
            ...
            arweave_metrics:gauge_dec(mining_server_task_queue_len, [TaskType]),
            case is_session_valid(State, Candidate) of
                true ->
...
add_task({TaskType, Candidate, _ExtraArgs} = Task, State) ->
    ...
    arweave_metrics:gauge_inc(mining_server_task_queue_len, [TaskType]),
    arweave_metrics:gauge_inc(mining_server_tasks, [TaskType]),
...
report_chunk_cache_metrics(#state{chunk_cache = ChunkCache, partition_number = Partition} = State) ->
    arweave_metrics:gauge_set(mining_server_chunk_cache_size, [Partition, "total"], ar_mining_cache:cache_size(ChunkCache)),
    case ar_mining_cache:reserved_size(ChunkCache) of
        {ok, ReservedSize} -> arweave_metrics:gauge_set(mining_server_chunk_cache_size, [Partition, "reserved"], ReservedSize);
        {error, Reason} ->
