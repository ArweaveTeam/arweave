...
update_rating_test() ->
    ...
    update_rating(Peer1, 1000, 100, false),
    assert_performance(#performance{
                          average_success = 0.9312 },
                       get_or_init_performance(Peer1)),

    %% Test successful transfer; ratings are derived on read.
    update_rating(Peer1, 1000, 100, true),
    assert_performance(#performance{
                          total_bytes = 100,
            ...
            average_success = 0.9336 },
                       get_or_init_performance(Peer1)),
    ?assertEqual(0.0934,
        round(get_peer_rating(lifetime, get_or_init_performance(Peer1)), 4)),
    ?assertEqual(0.0047,
        round(get_peer_rating(current, get_or_init_performance(Peer1)), 4)),

    %% A smaller transfer moves the throughput EWMA by its own delivered rate
    %% (no concurrency scaling).
    update_rating(Peer1, 1000, 50, true),
    assert_performance(#performance{
                          total_bytes = 150,
    ...
    %% A second peer's ratings are independent.
    update_rating(Peer2, 1000, 100, true),
    assert_performance(#performance{
                          total_bytes = 100,
                          total_throughput = 0.1,
                          total_transfers = 1,
    ...
    ?assertEqual(0.1,
        round(get_peer_rating(lifetime, get_or_init_performance(Peer2)), 4)),
    ?assertEqual(0.005,
        round(get_peer_rating(current, get_or_init_performance(Peer2)), 4)).
...
legacy_performance_load_test() ->
    ...
    try
        load_peer({Peer, {performance, 1000, 0, Transfers, 0, Rating}}),
        Loaded = get_or_init_performance(Peer),
        ?assertEqual(Rating, get_peer_rating(lifetime, Loaded)),
        ?assertEqual(Rating, get_peer_rating(current, Loaded)),
        load_peer({Peer, {performance, 1000, 0, Transfers, 0, Rating, 66}}),
        Loaded2 = get_or_init_performance(Peer),
        ?assertEqual(66, Loaded2#performance.release),
        ?assertEqual(Rating, get_peer_rating(lifetime, Loaded2))
    after
...
%% Goodput sampling moved to ar_sync_peer, scoped to the scheduler's active-peer
%% set. See the update_delivery_test there.
