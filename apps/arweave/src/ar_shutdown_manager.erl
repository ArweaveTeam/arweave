...
%%
%% Two modes are currently available: soft and hard. The soft mode
...
%%
%% The hard mode is setting linger and close the socket. This is not
...
stop_connection(State = #{ socket := Socket, mode := soft }) ->
	?LOG_DEBUG([{state, State}, {pid, self()}]),
	case State of
		#{ info := #{ states := [closed] }} ->
			stop;
...
stop_connection(State = #{ socket := Socket, mode := hard }) ->
	?LOG_DEBUG([{state, State}, {pid, self()}]),
	case State of
		#{ info := #{ states := [closed] }} ->
			stop;
