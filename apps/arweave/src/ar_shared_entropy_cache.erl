%%% @doc The module for managing the cache of entropies for
%%% the 2.9 replication scheme.
-module(ar_shared_entropy_cache).
...
-export([get/1, allocate_space/2, put/3]).
...
%% @doc Make sure the cache has enough space to store Size worth of elements such that
%% the total size does not exceed MaxSize. In other words, if you want to store new
%% elements with the total size Size, call allocate_space(Size, MaxSize) then
%% call put/3 to store new elements.
-spec allocate_space(
		Size :: non_neg_integer(),
		MaxSize :: non_neg_integer()
) -> ok.
allocate_space(Size, MaxSize) ->
	gen_server:cast(?MODULE, {allocate_space, Size, MaxSize}).
...
handle_cast({allocate_space, Size, MaxSize}, State) ->
	Table = replica_2_9_entropy_cache,
	OrderedKeyTable = replica_2_9_entropy_cache_ordered_keys,
	allocate_space(Size, MaxSize, Table, OrderedKeyTable),
	{noreply, State};
...
cache_test() ->
	...
	?assertEqual(0, get_fetched_key_count(Table, some_key)),
	allocate_space(64, 128, Table, OrderedKeyTable),
	...
	allocate_space(128, 128, Table, OrderedKeyTable),
	...
	allocate_space(0, 128, Table, OrderedKeyTable),
	?assertEqual(not_found, get(some_key, Table)),
	?assertEqual({ok, some_other_value}, get(some_other_key, Table)),
	?assertEqual({ok, yet_another_value}, get(yet_another_key, Table)),
	allocate_space(64, 128, Table, OrderedKeyTable),
