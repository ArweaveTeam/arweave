...
get_encrypted_block(Peer, Hash) when is_binary(Hash) ->
	handle_encrypted_block_response(
		ar_http:req(#{
			method => get,
			peer => Peer,
			path => "/block/hash/" ++ binary_to_list(ar_util:encode(Hash)) ++ "/encrypted",
			headers => p2p_headers()
		})
	).
...
get_block_subfield(Peer, Hash, Subfield) when is_binary(Hash) ->
	handle_block_field_response(
		ar_http:req(#{
			method => get,
			peer => Peer,
			path => "/block/hash/" ++ binary_to_list(ar_util:encode(Hash)) ++ "/" ++ Subfield,
			headers => p2p_headers()
		})
	);
...
get_block_subfield(Peer, Height, Subfield) when is_integer(Height) ->
	handle_block_field_response(
		ar_http:req(#{
			method => get,
			peer => Peer,
			path => "/block/height/" ++ integer_to_list(Height) ++ "/" ++ Subfield,
			headers => p2p_headers()
		})
	).
...
get_wallet_list(Peer, Hash) ->
	Response =
		ar_http:req(#{
			method => get,
			peer => Peer,
			path => "/block/hash/" ++ binary_to_list(ar_util:encode(Hash)) ++ "/wallet_list",
			headers => p2p_headers()
		}),
	case Response of
		{ok, {{<<"200">>, _}, _, Body, _, _}} ->
...
get_encrypted_full_block(Peer, Hash) when is_binary(Hash) ->
	handle_encrypted_full_block_response(
		ar_http:req(#{
			method => get,
			peer => Peer,
			path => "/block/hash/" ++ binary_to_list(ar_util:encode(Hash)) ++ "/all/encrypted",
			headers => p2p_headers()
		})
	).
...
get_tx_data(Peer, Hash) ->
	{ok, {{<<"200">>, _}, _, Body, _, _}} =
		ar_http:req(#{
			method => get,
			peer => Peer,
			path => "/" ++ binary_to_list(ar_util:encode(Hash)),
			headers => p2p_headers(),
			timeout => 10 * 1000
		}),
	Body.
