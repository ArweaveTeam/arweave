...
%%  - the top arweave_constants:get_consensus_window_size() blocks of the
%%    longest chain
%% tip -> BlockHash
...
%% arweave_constants:get_consensus_window_size() blocks of the longest chain
%% and the number of blocks in this list that are not on chain yet.
%%
...
get_longest_chain_block_txs_pairs(Tab, H, N, PrevStatus, PrevH, Pairs, NotOnChainCount) ->
    case ets:lookup(Tab, {block, H}) of
        [{_, {B, {not_validated, awaiting_nonce_limiter_validation}, _Timestamp,
              _Children}}] ->
            get_longest_chain_block_txs_pairs(
                Tab, B#block.previous_block,
...
update_longest_chain_cache(Tab) ->
    ...
    Result = get_longest_chain_block_txs_pairs(
        Tab, H, arweave_constants:get_consensus_window_size(),
        none, none, [], 0
    ),
