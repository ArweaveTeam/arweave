...
handle_cast(report_performance, #state{ io_threads = IOThreads, session = Session } = State) ->
	...
	case Seconds > 0 of
		true ->
			Avg = Sum / Seconds / 4,
			?LOG_INFO([{event, mining_performance_report}, {total_avg, Avg},
					{total_current, Current / 4}]),
			Str = io_lib:format("~nMining performance report:~nTotal avg: ~.2f MiB/s, "
					"current: ~.2f MiB/s.~n", [Avg, Current / 4]),
	...
	case ar_nonce_limiter:get_last_step_checkpoints(StartIntervalNumber, StepNumber,
		...
		LastStepCheckpoints ->
			case validate_solution({NonceLimiterOutput, PartitionNumber, Seed, ReplicaID,
				...
				false ->
					?LOG_INFO([{event, found_invalid_solution}, {zone, PartitionNumber},
							{step_number, StepNumber},
							{mining_address, ar_util:encode(ReplicaID)},
				...
				true ->
					...
					?LOG_INFO([{event, found_mining_solution},
							{zone, PartitionNumber}, {step_number, StepNumber},
							{mining_address, ar_util:encode(ReplicaID)},
...
validate_solution(Args) ->
	...
	case binary:decode_unsigned(H1, big) > Diff of
		true ->
			...
			ar_poa:validate(BlockStart1, RecallByte1, TXRoot1, BlockSize1, PoA1, 0,
					?STRICT_DATA_SPLIT_THRESHOLD, RewardAddr);
		false ->
			...
			case binary:decode_unsigned(H2, big) > Diff of
				...
				true ->
					...
					ar_poa:validate(BlockStart2, RecallByte2, TXRoot2, BlockSize2, PoA2, 0,
							?STRICT_DATA_SPLIT_THRESHOLD, RewardAddr)
			end
	end.
