...
unsynced_intervals(Start, End, StoreID) ->
    ...
    KeptIntervals = ar_footprint_limit:kept_intervals(
        Start, End, ar_footprint_limit:get(StoreID)),
    ar_intervals:intersection(KeptIntervals,
        ar_intervals:outerjoin(BlacklistedIntervals, UnsyncedIntervals)).

unsynced_footprint_intervals(Partition, Footprint, StoreID) ->
    case Footprint < ar_footprint_limit:get(StoreID) of
        true ->
