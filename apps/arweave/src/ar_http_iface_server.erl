...
terminate_connection_init(Connection, Delay) ->
	...
	try inet:peername(Socket) of
		{ok, {PeerAddress, PeerPort}} ->
			...
			?LOG_WARNING(#{ connection => Peer, socket => Connection, reason => "shutdown", msg => "shutdown (read-only)"}),
...
terminate_connection_loop(State) ->
	...
	receive
		{timeout, close} ->
			...
			% socket time.
			?LOG_WARNING(#{ connection => Peer, socket => Connection, reason => "shutdown", msg => "closed (timeout)"}),
		...
		{timeout, kill} ->
			...
			% monitoring.
			?LOG_WARNING(#{ connection => Peer, socket => Connection, reason => "shutdown", msg => "killed (timeout)"}),
			ranch_tcp:setopts(Socket, [{linger, {true, 0}}]),
			ranch_tcp:close(Socket),
			terminate_connection_loop(State);

		{'DOWN', _Ref, process, Connection, _} ->
			?LOG_INFO(#{ connection => Peer, socket => Connection, reason => "shutdown", msg => "terminated"});
	Msg ->
