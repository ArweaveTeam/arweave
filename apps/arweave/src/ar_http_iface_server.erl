...
handle_info(Msg = {'EXIT', From, Reason}, State) ->
...
terminate(Reason, _State) ->
	...
	?LOG_INFO(#{ stop => ?MODULE, reason => Reason }).
...
terminate_connection_init(Connection, Delay) ->
	try
		...
		% create the first timer for the close timeout
		{ok, CloseRef} = timer:send_after(Delay, {timeout, close}),
		?LOG_DEBUG(#{ timer => CloseRef, msg => "tcp socket close timeout" }),

		% create the second timer for the kill timeout
		{ok, KillRef} = timer:send_after(Delay*2, {timeout, kill}),
		...
		?LOG_INFO(#{ connection => Peer, socket => Connection, reason => "shutdown", msg => "shutdown (read-only)"}),
		ranch_tcp:shutdown(Socket, write),

		% let prepare the state used for the termination loop procedure.
		State = #{
			close_reference => CloseRef,
...
terminate_connection_loop(State) ->
	AfterDelay = maps:get(delay, State)*3,
	...
	receive
		{'DOWN', Ref, port, Socket, _} ->
			...
			% now can be stopped.
			?LOG_INFO(#{ connection => Peer, socket => Connection, reason => "shutdown", msg => "terminated (normal)"}),
			ok;

		{timeout, close} ->
			% the first timeout simply close the socket.
			?LOG_WARNING(#{ connection => Peer, socket => Connection, reason => "shutdown", msg => "close (timeout)"}),
		...
		{timeout, kill} ->
			...
			% monitoring.
			?LOG_WARNING(#{ connection => Peer, socket => Connection, reason => "shutdown", msg => "kill (timeout)"}),
	...
	after
		AfterDelay ->
			...
			% loop stopped.
			?LOG_ERROR(#{ connection => Peer, socket => Connection, reason => "shutdown", msg => "can't kill"}),
