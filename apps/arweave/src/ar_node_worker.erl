...
validate_trusted_peers(Peers) ->
	...
	case ValidPeers of
		[] ->
			...
			init:stop(1);
		_ ->
			%% Rewrite the trusted-peer role so unreachable or
			%% wrong-network peers are evicted. Runs during sup tree
			%% boot, before `arweave_config:runtime/0' freezes static
			%% specs.
