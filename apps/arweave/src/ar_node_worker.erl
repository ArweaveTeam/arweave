...
may_be_initialize_nonce_limiter([#block{ height = Height } = B | Blocks], BI) ->
	case Height + 1 == ar_fork:height_2_7() of
		true ->
			{Seed, PartitionUpperBound, _TXRoot} = ar_node:get_partition_upper_bound(BI),
...
handle_initialized(State) ->
	...
	ar_data_sync:join(RecentBI, B#block.packing_2_5_threshold, B#block.packing_2_6_threshold),
...
pack_block_with_transactions(#block{ height = Height } = B, PrevB) ->
	...
	ValidTXs = ar_tx_replay_pool:pick_txs_to_mine({BlockAnchors, RecentTXMap, Height - 1,
			Rate, B#block.timestamp, Accounts, TXs}),
	BlockSize =
		lists:foldl(
			fun(TX, Acc) ->
	...
	{Reward, EndowmentPool} =
		ar_pricing:get_miner_reward_and_endowment_pool({PrevB#block.reward_pool, ValidTXs,
				B#block.reward_addr, WeaveSize, Height, B#block.timestamp, Rate}),
	...
	Accounts3 = ar_node_utils:apply_txs(Accounts2, ValidTXs, Height - 1),
	Accounts4 = ar_node_utils:apply_mining_reward(Accounts3, RewardAddr, Reward),
	{ok, RootHash} = ar_wallets:add_wallets(PrevB#block.wallet_list, Accounts4, RewardAddr,
			Height),
	B#block{
...
validate_wallet_list(B, WalletList, RewardPool, Rate, Height) ->
	case ar_wallets:apply_block(B, WalletList, RewardPool, Rate, Height) of
		...
		{error, invalid_miner_reward} ->
			?LOG_WARNING([{event, received_invalid_block},
					{validation_error, invalid_miner_reward},
					{h, ar_util:encode(B#block.indep_hash)}]),
			error;
...
apply_validated_block(State, B, PrevBlocks, Orphans, RecentBI, BlockTXPairs) ->
	...
	B2 =
		case B#block.height + 1 == ar_fork:height_2_7() of
			true ->
				Info = ar_nonce_limiter:get_or_init_nonce_limiter_info(B, RecentBI),
				B3 = B#block{ nonce_limiter_info = Info },
				ar_events:send(node_state, {validated_pre_fork_2_7_block, B3}),
				B3;
			_ ->
				B
		end,
...
apply_validated_block2(State, B, PrevBlocks, Orphans, RecentBI, BlockTXPairs) ->
	...
	lists:foldl(
		fun (CurrentB, start) ->
			...
			(CurrentB, CurrentPrevB) ->
				...
				ok = ar_wallets:set_current(PrevWallets, Wallets, Addr, Height, PruneDepth2),
				CurrentB
		end,
		start,
	...
	ar_data_sync:add_tip_block(B#block.packing_2_5_threshold, B#block.packing_2_6_threshold,
			BlockTXPairs, lists:sublist(RecentBI, ?STORE_BLOCKS_BEHIND_CURRENT * 2)),
...
record_economic_metrics(B) ->
	case B#block.height >= ar_fork:height_2_5() of
		false ->
			ok;
		true ->
			record_economic_metrics2(B)
	end.

record_economic_metrics2(B) ->
	prometheus_gauge:set(network_hashrate, ?MAX_DIFF div (?MAX_DIFF - B#block.diff)),
	...
	{RateDivisor, RateDividend} = ar_pricing:get_expected_min_decline_rate(B#block.timestamp,
	...
	{RateDivisor2, RateDividend2} = ar_pricing:get_expected_min_decline_rate(B#block.timestamp,
			Period_200_Years, B#block.reward_pool, B#block.weave_size, {1, 10},
			B#block.height),
	prometheus_gauge:set(expected_minimum_200_years_storage_costs_decline_rate_10_usd_ar,
...
start_from_block_index(BI) ->
	case length(BI) - 1 - ?STORE_BLOCKS_BEHIND_CURRENT > ar_fork:height_2_7() of
		true ->
			ok;
		_ ->
			ar_randomx_state:init(BI, [])
