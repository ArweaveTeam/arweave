...
validate_spora(Args) ->
	{BDS, Nonce, Timestamp, Height, Diff, PrevH, PartitionUpperBound, StrictDataSplitThreshold,
			SPoA} = Args,
...
validate_spoa(Args) ->
	{RecallByte, SPoA, SolutionHash, HashPreimage, StrictDataSplitThreshold} = Args,
...
update_txs(
	...
	ValidTXs =
		ar_tx_replay_pool:pick_txs_to_mine({
			...
			0, % The redenomination height is always 0 before the 2.6 fork.
			Rate,
			0, % The future "price per byte minute"; does not play a role here yet.
			1, % The Kryder+ rate multiplier is always 1 before the 2.6 fork.
			1, % The denomination code is always 1 before the 2.6 fork.
			NextBlockTimestamp,
	...
	Wallets2 =
		%% The denomination is always 1 before the fork 2.6.
		ar_node_utils:apply_mining_reward(AppliedTXsWallets, RewardAddr, FinderReward, 1),
	{ok, RootHash2} = ar_wallets:add_wallets(CurrentB#block.wallet_list, Wallets2, Height, 1),
	CandidateB2 =
		CandidateB#block{
...
start_hashing_threads2(S) ->
	#state{ current_block = #block{ indep_hash = PrevH },
			...
			io_threads = IOThreads, stage_one_hasher = Hasher,
			stage_two_hasher = StageTwoHasher,
	...
	%% recall bytes, schedule new chunk reads, and pass fetched chunks to the hashing
	%% processors while all the dirty CPU schedulers are busy.
		...
		{solution, Nonce, H0, Timestamp, Hash} ->
			case maps:get(Timestamp, BlocksByTimestamp, not_found) of
				...
				{#block{ timestamp = Timestamp } = B, BDS} ->
					...
					case get_spoa(H0, PrevH, PartitionUpperBound, Height, PackingThreshold) of
...
io_thread(SearchInRocksDB) ->
	[{_, {SessionRef, SessionTimestamp, PackingThreshold,
			Height}}] = ets:lookup(mining_state, session),
	receive
...
io_thread_read_chunk(Args) ->
	{EncodedByte, H0, Nonce, HashingThread, Entropy, Timestamp, Diff, SessionRef,
			_PackingThreshold, SearchInRocksDB, _Height} = Args,
...
read_chunk(Byte, SearchInRocksDB) ->
	case SearchInRocksDB of
		false ->
			case ar_sync_record:is_recorded(Byte + 1, spora_2_5, ar_data_sync, "default") of
				false ->
				...
				true ->
					case ar_chunk_storage:get(Byte, "default") of
						not_found ->
							{error, chunk_not_found};
						{_Offset, Chunk} ->
							case ar_sync_record:is_recorded(Byte + 1, spora_2_5, ar_data_sync,
									"default") of
								true ->
									{ok, Chunk};
								false ->
		...
		true ->
			Options = #{ packing => spora_2_5, pack => false },
...
get_spoa(H0, PrevH, PartitionUpperBound, Height, PackingThreshold) ->
	case pick_recall_byte(H0, PrevH, PartitionUpperBound) of
		{error, weave_size_too_small} ->
			{#poa{}, undefined};
		{ok, RecallByte} ->
...
start_io_threads() ->
	...
	{ok, Config} = application:get_env(arweave, config),
	ets:insert(mining_state, {session,
			{make_ref(), os:system_time(second), not_set, not_set}}),
...
test_timestamp_refresh() ->
	...
	[B0] = ar_weave:init([], ar_retarget:switch_to_linear_diff(14)),
	ar_test_node:start(B0),
	B = B0,
	ok = ar_events:subscribe(block),
	Threads = start_io_threads(),
...
assert_mine_output(B, TXs) ->
	receive
		{event, block, {mined, NewB, MinedTXs, BH}} ->
			...
			{H0, Entropy} = spora_h0_with_entropy(BDS, Nonce, Height),
			?assertEqual(element(1, spora_solution_hash_with_entropy(PrevH, Timestamp, H0,
					Chunk, Entropy, Height)), NewB#block.hash),
