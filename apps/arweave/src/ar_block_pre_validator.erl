...
handle_cast(pre_validate, #state{ pqueue = Q, size = Size, ip_timestamps = IPTimestamps,
	...
	case gb_sets:is_empty(Q) of
		true ->
			...
			{noreply, State};
		false ->
			{{_, {B, BDS, PrevB, SolutionResigned, Peer, Timestamp, ReadBodyTime, BodySize}},
					...
					{true, IPTimestamps2} ->
						case throttle_by_solution_hash(B#block.hash, HashTimestamps) of
							{true, HashTimestamps2} ->
								...
								pre_validate_pow_or_partition_number(B, BDS, PrevB,
...
handle_cast({enqueue, {B, BDS, PrevB, SolutionResigned, Peer, Timestamp, ReadBodyTime,
		BodySize}}, State) ->
	#state{ pqueue = Q, size = Size } = State,
	Priority = priority(B, Peer),
	Size2 = Size + BodySize,
	Q2 = gb_sets:add_element({Priority, {B, BDS, PrevB, SolutionResigned, Peer, Timestamp,
			...
			case ar_block:verify_cumulative_diff(B, PrevB) of
				...
				true ->
					case SolutionResigned of
						true ->
							gen_server:cast(?MODULE, {enqueue, {B, BDS, PrevB, true, Peer,
									Timestamp, ReadBodyTime, BodySize}});
						false ->
							pre_validate_quick_pow(B, BDS, PrevB, false, Peer, Timestamp,
									ReadBodyTime, BodySize)
					end
			end;
		false ->
...
pre_validate_quick_pow(B, BDS, PrevB, SolutionResigned, Peer, Timestamp, ReadBodyTime,
		BodySize) ->
		...
		_ ->
			case binary:decode_unsigned(SolutionHash, big) > Diff of
				false ->
					...
					ok;
				true ->
					gen_server:cast(?MODULE, {enqueue, {B, BDS, PrevB, SolutionResigned, Peer,
							Timestamp, ReadBodyTime, BodySize}})
			end
	end.
...
get_prev_nonce_limiter_info(#block{ indep_hash = PrevH, height = PrevHeight } = PrevB) ->
	Fork_2_7 = ar_fork:height_2_7(),
	true = PrevHeight + 1 >= Fork_2_7,
	case PrevHeight + 1 == Fork_2_7 of
		true ->
...
pre_validate_pow_or_partition_number(B, BDS, PrevB, SolutionResigned, Peer, Timestamp,
		ReadBodyTime, BodySize) ->
	case B#block.height >= ar_fork:height_2_7() of
		false ->
	...
	case ar_data_sync:get_chunk(RecallByte2 + 1, #{ pack => true,
		...
		{ok, #{ chunk := Chunk, data_path := DataPath, tx_path := TXPath }} ->
			...
			B2 = B#block{ poa2 = #poa{ chunk = Chunk, data_path = DataPath,
					tx_path = TXPath } },
			pre_validate_pow_2_7(B2, PrevB, PartitionUpperBound, Peer, Timestamp,
					ReadBodyTime, BodySize);
		_ ->
...
pre_validate_may_be_fetch_second_chunk(B, PrevB, PartitionUpperBound, Peer, Timestamp,
			ReadBodyTime, BodySize) ->
	pre_validate_pow_2_7(B, PrevB, PartitionUpperBound, Peer, Timestamp, ReadBodyTime,
			BodySize).
	...
	case ar_data_sync:get_chunk(RecallByte + 1, Options) of
		{ok, #{ chunk := Chunk, data_path := DataPath, tx_path := TXPath }} ->
			...
			B2 = B#block{ poa = #poa{ chunk = Chunk, tx_path = TXPath,
					data_path = DataPath } },
			pre_validate_pow_or_indep_hash(B2, BDS, PrevB, Peer, Timestamp, ReadBodyTime,
					BodySize);
		_ ->
...
drop_tail(Q, Size) when Size =< ?MAX_PRE_VALIDATION_QUEUE_SIZE ->
	{Q, 0};
drop_tail(Q, Size) ->
	{{_Priority, {_, _, _, _, _, _, _, BodySize}}, Q2} = gb_sets:take_smallest(Q),
