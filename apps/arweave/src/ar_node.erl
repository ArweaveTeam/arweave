...
handle_call(is_joined, _From, State) ->
	{reply, State#state.joined, State};
...
handle_call(get_ready_for_mining_txs, _From, State) ->
	MinedTXs = maps:fold(
		fun
...
handle_info({join, BI, Blocks}, State) ->
	...
	State1 = State#state{joinBI = BI, joinBlocks = Blocks},
	{noreply, State1};

handle_info(wallet_ready, State) ->
	BI = State#state.joinBI,
	Blocks = State#state.joinBlocks,
