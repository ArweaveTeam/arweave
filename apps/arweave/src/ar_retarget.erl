...
calculate_difficulty(OldDiff, TS, Last, Height, PrevTS) ->
	...
	case Height of
		_ when Height == Fork_2_7 ->
			calculate_difficulty_with_drop(OldDiff, TS, Last, Height, PrevTS, ?DIFF_DROP_2_7);
		_ when Height == Fork_2_6 ->
			calculate_difficulty_with_drop(OldDiff, TS, Last, Height, PrevTS, ?DIFF_DROP_2_6);
...
calculate_difficulty_with_drop(OldDiff, TS, Last, Height, PrevTS, Coeff) ->
	TargetTime = ?RETARGET_BLOCKS * ?TARGET_TIME,
	ActualTime = TS - Last,
	Step = 10 * 60,
	%% Drop the difficulty Coeff times right away, then drop extra Coeff times
	%% for every 10 minutes passed.
	ActualTime2 = ActualTime * Coeff * ar_fraction:pow(Coeff, max(TS - PrevTS, 0) div Step),
	MaxDiff = ?MAX_DIFF,
	MinDiff = ar_mine:min_difficulty(Height),
