...
handle_info({entropy, BucketEndOffset, Entropies}, State) ->
	#state{ 
		range_start = RangeStart, iteration_end = IterationEnd,
		...
		false ->
			...
			case IsRecorded of
				...
				false ->
					...
					%% Generate 256 MiB of entropy and repack the corresponding chunks
					ar_entropy_gen:generate_entropies(
						StoreID, RewardAddr, BucketEndOffset, self()),
					repack_chunks(EntropyOffsets, State2)
			end
	end.

repack_chunks([], State) ->
	log_debug(repack_batch_chunks_read, State#state.store_id, [
	...
	State;
repack_chunks([BucketEndOffset | EntropyOffsets], #state{ range_start = RangeStart } = State) 
		when BucketEndOffset < RangeStart ->
	log_debug(repack_chunks_skipping_bucket_end_offset, State#state.store_id, [
		{bucket_end_offset, BucketEndOffset},
		{range_start, State#state.range_start},
		{range_end, State#state.range_end},
		{iteration_end, State#state.iteration_end}
...
repack_chunks([BucketEndOffset | EntropyOffsets], State) ->
	#state{ target_packing = TargetPacking, store_id = StoreID } = State,
	ChunkInfo = read_chunk(BucketEndOffset, TargetPacking, StoreID),
	ChunkInfo2 = repack_chunk(ChunkInfo),
	State2 = cache_chunk_info(ChunkInfo2, State),
	repack_chunks(EntropyOffsets, State2).

read_chunk(BucketEndOffset, TargetPacking, StoreID) ->
	Byte = ar_chunk_storage:get_chunk_byte_from_bucket_end(BucketEndOffset),
	case ar_data_sync:get_chunk_by_byte(Byte, StoreID) of
		{ok, _Key, FullMetadata} ->
			{AbsoluteOffset, ChunkDataKey, TXRoot, DataRoot, TXPath,
				RelativeOffset, ChunkSize} = FullMetadata,
			PaddedEndOffset = ar_block:get_chunk_padded_offset(AbsoluteOffset),

			IsTooSmall = (
				ChunkSize /= ?DATA_CHUNK_SIZE andalso AbsoluteOffset =< ?STRICT_DATA_SPLIT_THRESHOLD
			),
			...
			read_chunk2(ChunkInfo, TargetPacking, StoreID);
		{error, invalid_iterator} ->
			...
			log_debug(no_chunk_to_repack, ChunkInfo, StoreID, []),
			ChunkInfo;
		{error, Reason} ->
			ChunkInfo = #chunk_info{
				status = no_chunk,
			...
			log_warning(failed_to_read_chunk_metadata, ChunkInfo, StoreID, [{reason, Reason}]),
			ChunkInfo
	end.
...
cache_chunk_info(ChunkInfo, State) ->
	% log_debug(caching_chunk_info, ChunkInfo, State#state.store_id, []),
