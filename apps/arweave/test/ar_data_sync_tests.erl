...
mines_off_only_last_chunks_test_() ->
	test_with_mocked_functions([{ar_fork, height_2_6, fun() -> 0 end},
		{ar_fork, height_2_7, fun() -> 0 end}], fun test_mines_off_only_last_chunks/0).
...
mines_off_only_second_last_chunks_test_() ->
	test_with_mocked_functions([{ar_fork, height_2_6, fun() -> 0 end},
		{ar_fork, height_2_7, fun() -> 0 end}], fun test_mines_off_only_second_last_chunks/0).
...
test_packs_chunks_depending_on_packing_threshold() ->
	...
	LastB_2_5 = read_block_when_stored(element(1, lists:nth(10, lists:reverse(BILast)))),
	LastB = lists:foldl(
		fun(Height, PrevB) ->
			H = element(1, lists:nth(Height + 1, lists:reverse(BILast))),
			B = read_block_when_stored(H),
			?assertEqual(max(0,
					?PACKING_2_6_THRESHOLD_START - (Height - 10) * (?DATA_CHUNK_SIZE) * 120),
