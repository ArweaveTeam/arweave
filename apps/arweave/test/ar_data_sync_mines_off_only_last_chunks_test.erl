...
mines_off_only_last_chunks_test_() ->
	[
		{"mines_off_only_last_chunks POST /chunk",
			test_with_mocked_functions([{ar_fork, height_2_6, fun() -> 0 end}, mock_reset_frequency()],
				fun() -> test_mines_off_only_last_chunks(post_chunk) end)},
		{"mines_off_only_last_chunks POST /chunk/OFFSET",
			test_with_mocked_functions([{ar_fork, height_2_6, fun() -> 0 end}, mock_reset_frequency()],
				fun() -> test_mines_off_only_last_chunks(post_chunk_by_offset) end)}
	].

mock_reset_frequency() ->
	{ar_nonce_limiter, get_reset_frequency, fun() -> 5 end}.

test_mines_off_only_last_chunks(PostMode) ->
	...
	lists:foreach(
		fun(Height) ->
			...
			AbsoluteEndOffset = B#block.weave_size,
