...
test_standalone_v1_denomination0_tx_is_dropped() ->
	...
	[{H, _, _} | _] = ar_test_node:wait_until_height(main, 1),
...
tx_path(ID) ->
	"/tx/" ++ binary_to_list(ar_util:encode(ID)).

tx2_path(ID) ->
	"/tx2/" ++ binary_to_list(ar_util:encode(ID)).

unconfirmed_tx_path(ID) ->
	"/unconfirmed_tx/" ++ binary_to_list(ar_util:encode(ID)).
