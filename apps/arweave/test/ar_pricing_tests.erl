...
test_auto_redenomination_and_endowment_debt() ->
	...
	[B0] = ar_weave:init([
		{ar_wallet:to_address(Pub1), 20000000000, <<>>},
	...
	?assert(?RESET_KRYDER_PLUS_LATCH_THRESHOLD == 1000000000),
	?assert(?TOTAL_SUPPLY == 15000000000),
	?assert(?REDENOMINATION_THRESHOLD == 13500000000),
	...
	?assertEqual(ar_pricing:get_price_per_gib_minute(lists:sublist(B3#block.price_history, 3),
	...
	?assertEqual(20000000000, get_balance(master, Pub1)),
	?assert(lists:member(get_optimistic_tx_price(master, 1024), [Fee * 2, Fee * 2 + 1])),
	...
	?assertEqual(
		min(ar_pricing:get_price_per_gib_minute(lists:sublist(B5#block.price_history, 3), 1),
	...
	{Reward10, _} = get_tx_price(master, 0, <<"non-existent-address">>),
	ar_node:mine(),
