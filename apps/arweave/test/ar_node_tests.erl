...
-import(ar_test_node, [
		...
		slave_call/3, disconnect_from_slave/0, sign_tx/2, wait_until_joined/0,
...
%	lists:foreach(
%		fun({Addr, Balance}) ->
%			?assertEqual(Balance, get_balance_by_address(Addr, slave),
%					Title ++ "Address: " ++ binary_to_list(ar_util:encode(Addr))
%					++ " Height: " ++ integer_to_list(Height)),
%			?assertEqual(Balance, get_balance_by_address(Addr, master),
%					Title ++ "Address: " ++ binary_to_list(ar_util:encode(Addr))
%					++ " Height: " ++ integer_to_list(Height))
%		end,
