...
one_wait_per_process_test_() ->
	with_semaphore_(one_wait_per_process_sem, 4, fun() ->
		?assertEqual(ok, ar_semaphore:acquire(one_wait_per_process_sem, 600000)),
		?assertEqual({error, process_already_waiting}, ar_semaphore:acquire(one_wait_per_process_sem, 600000))
	end).
...
wait_for_one_process_at_a_time_test_() ->
	with_semaphore_(wait_for_one_process_at_a_time_sem, 1, fun() ->
		TestPid = self(),
		spawn_link(fun() ->
			ok = ar_semaphore:acquire(wait_for_one_process_at_a_time_sem, 600000),
			timer:sleep(200),
			TestPid ! p1_done
		end),
		spawn_link(fun() ->
...
wait_for_two_processes_at_a_time_test_() ->
	with_semaphore_(wait_for_two_processes_at_a_time_sem, 2, fun() ->
		TestPid = self(),
		spawn_link(fun() ->
			ok = ar_semaphore:acquire(wait_for_two_processes_at_a_time_sem, 600000),
			timer:sleep(400),
			TestPid ! p1_done
		end),
		spawn_link(fun() ->
