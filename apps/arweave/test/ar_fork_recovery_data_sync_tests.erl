...
test_fork_recovery(Split) ->
    ...
    ?debugFmt("Posting tx to main ~s.~n", [arweave_util:encode(TX1#tx.id)]),
    B1 = ar_test_node:post_and_mine(#{ miner => main, await_on => peer1 }, [TX1]),
    ?debugFmt("Mined block ~s, height ~B.~n", [arweave_util:encode(B1#block.indep_hash),
            B1#block.height]),
    ...
    ?debugFmt("Posting tx to peer1 ~s.~n", [arweave_util:encode(PeerTX2#tx.id)]),
    ?debugFmt("Posting tx to peer1 ~s.~n", [arweave_util:encode(PeerTX3#tx.id)]),
    PeerB2 = ar_test_node:post_and_mine(#{ miner => peer1, await_on => peer1 },
            [PeerTX2, PeerTX3]),
    ?debugFmt("Mined block ~s, height ~B.~n", [arweave_util:encode(PeerB2#block.indep_hash),
            PeerB2#block.height]),
    {MainTX2, MainChunks2} = ar_test_data_sync:tx(Wallet, {Split, 1}, v2, ?AR(10)),
    ?debugFmt("Posting tx to main ~s.~n", [arweave_util:encode(MainTX2#tx.id)]),
    MainB2 = ar_test_node:post_and_mine(#{ miner => main, await_on => main },
            [MainTX2]),
    ?debugFmt("Mined block ~s, height ~B.~n", [arweave_util:encode(MainB2#block.indep_hash),
            MainB2#block.height]),
    ...
    ?debugFmt("Posting tx to peer1 ~s.~n", [arweave_util:encode(PeerTX4#tx.id)]),
    PeerB3 = ar_test_node:post_and_mine(#{ miner => peer1, await_on => peer1 },
            [PeerTX4]),
    ?debugFmt("Mined block ~s, height ~B.~n", [arweave_util:encode(PeerB3#block.indep_hash),
            PeerB3#block.height]),
    ...
    ?debugFmt("Posting tx to main ~s.~n", [arweave_util:encode(MainTX3#tx.id)]),
    MainB3 = ar_test_node:post_and_mine(#{ miner => main, await_on => main },
            [MainTX3]),
    ?debugFmt("Mined block ~s, height ~B.~n", [arweave_util:encode(MainB3#block.indep_hash),
            MainB3#block.height]),
    ...
    %% and gossip them.
    ?debugFmt("Posting tx to main ~s.~n", [arweave_util:encode(PeerTX2#tx.id)]),
    ?debugFmt("Posting tx to main ~s.~n", [arweave_util:encode(PeerTX4#tx.id)]),
    ...
    ?debugFmt("Mined block ~s, height ~B.~n", [arweave_util:encode(MainB4#block.indep_hash),
            MainB4#block.height]),
