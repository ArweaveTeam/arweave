-module(ar_poller_tests).
-test_peers([peer1]).

-include_lib("arweave/include/ar.hrl").
-include_lib("eunit/include/eunit.hrl").

polling_test_() ->
    ar_test_node:test_with_all_nodes_mocked([
        {ar_retarget, is_retarget_height, fun(_Height) -> false end},
        {ar_retarget, is_retarget_block, fun(_Block) -> false end}],
        fun test_polling/0).

test_polling() ->
    {_, Pub} = Wallet = ar_wallet:new(),
    [B0] = ar_weave:init([{ar_wallet:to_address(Pub), ?AR(10000), <<>>}]),
    ar_test_node:start(B0),
    ar_test_node:start_peer(peer1, B0),
    ar_test_node:disconnect_from(peer1),
    TXs =
        lists:map(
            fun(Height) ->
                SignedTX = ar_test_node:sign_tx(Wallet, #{ last_tx => ar_test_node:get_tx_anchor(peer1) }),
                ar_test_node:assert_post_tx_to_peer(peer1, SignedTX),
                ar_test_node:mine(peer1),
                ?assertMatch({ok, _}, ar_test_await:node_height(peer1, Height)),
                SignedTX
            end,
            lists:seq(1, 9)
        ),
    ar_test_node:connect_to_peer(peer1),
    ?assertMatch({ok, _}, ar_test_await:node_height(main, 9)),
    lists:foreach(
        fun(Height) ->
            {H, _, _} = ar_node:get_block_index_entry(Height),
            B = ar_test_await:block_stored(H),
            TX = lists:nth(Height, TXs),
            ?assertEqual([TX#tx.id], B#block.txs)
        end,
        lists:seq(1, 9)
    ),
    %% Make the nodes diverge. Expect one of them to fetch and apply the blocks
    %% from the winning fork.
    ar_test_node:disconnect_from(peer1),
    ar_test_node:mine(),
    ar_test_node:mine(peer1),
    {ok, [{MH11, _, _} | _]} = ar_test_await:node_height(main, 10),
    {ok, [{SH11, _, _} | _]} = ar_test_await:node_height(peer1, 10),
    ?assertNotEqual(SH11, MH11),
    ar_test_node:mine(),
    ar_test_node:mine(peer1),
    {ok, [{MH12, _, _} | _]} = ar_test_await:node_height(main, 11),
    {ok, [{SH12, _, _} | _]} = ar_test_await:node_height(peer1, 11),
    ?assertNotEqual(SH12, MH12),
    ar_test_node:mine(),
    ar_test_node:mine(peer1),
    {ok, [{MH13, _, _} | _] = MBI12} = ar_test_await:node_height(main, 12),
    {ok, [{SH13, _, _} | _] = SBI12} = ar_test_await:node_height(peer1, 12),
    ?assertNotEqual(SH13, MH13),
    BM13 = ar_block_cache:get(block_cache, MH13),
    BS13 = ar_test_node:remote_call(peer1, ar_block_cache, get, [block_cache, SH13]),
    CDiffM13 = BM13#block.cumulative_diff,
    CDiffS13 = BS13#block.cumulative_diff,
    ar_test_node:connect_to_peer(peer1),
    case CDiffM13 > CDiffS13 of
        true ->
            ?debugFmt("Case 1.", []),
            ?assertEqual(ok, ar_test_await:block_index_matches(peer1, MBI12)),
            ?assertMatch([{MH13, _, _} | _], ar_node:get_block_index());
        false ->
            case CDiffM13 < CDiffS13 of
                true ->
                    ?debugFmt("Case 2.", []),
                    ?assertEqual(ok, ar_test_await:block_index_matches(main, SBI12)),
                    ?assertMatch([{SH13, _, _} | _],
                            ar_test_node:remote_call(peer1, ar_node, get_block_index, []));
                false ->
                    ?debugFmt("Case 3.", []),
                    ar_test_node:mine(peer1),
                    {ok, [{MH14, _, _}, {MH13_1, _, _}, {MH12_1, _, _}, {MH11_1, _, _} | _]}
                    = ar_test_await:node_height(main, 13),
                    {ok, [{SH14, _, _} | _]} = ar_test_await:node_height(peer1, 13),
                    ?assertEqual(MH14, SH14),
                    ?assertEqual(SH13, MH13_1),
                    ?assertEqual(SH12, MH12_1),
                    ?assertEqual(SH11, MH11_1)
            end
    end.
