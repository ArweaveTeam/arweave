...
init_per_suite(Config) ->
	% clean the data dir before we start this suite if it wasn't by some reason
...
write_genesis_files(DataDir, B0, WalletList) ->
	% make sure if all required directories are exists
